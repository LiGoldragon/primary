# Primary Workspace — Extended Agent Instructions

Companion to `AGENTS.md`. The compact file carries the rules that
apply on every keystroke. This file carries rationale, longer
explanations, and discipline that isn't read every session — only when
the topic comes up.
