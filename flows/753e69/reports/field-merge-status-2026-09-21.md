...
| Horizon | Corrective remote main `b45d6ad48b5e` restores the typed Datomic `ClusterProposal`/`NodeService` producer with USB gateway and OpenCodeTesting. | Full workspace tests and a Prometheus-only Nix gate passed. The earlier ancestry merge `92d40e362393` was incomplete; consumers must pin this corrective producer and prove their own contracts before main integration. |
...
The initial Horizon ancestry merge `92d40e362393` lacked `lib/src/proposal.rs` and the authored `ClusterProposal` public type. The corrective `b45d6ad48b5e` reinstated that primary API, ported OpenCodeTesting into the typed service enum, and passed its own remote gate. Signal is being aligned to this exact revision; its earlier `37416e10c00c` test receipt does not prove compatibility with the correction. No consumer pin advances on the basis of the old-schema merge.
