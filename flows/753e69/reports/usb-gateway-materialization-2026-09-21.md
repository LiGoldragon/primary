...
`cargo fmt --all -- --check`, `cargo metadata --offline --locked --no-deps`, `git apply --check --reverse --whitespace=error`, and a Lojix Nix derivation evaluation passed. Cargo.lock has one `horizon-lib`, one `signal-lojix`, and one `meta-signal-lojix` source. The Signal `test-datom-contract` remote-only build reached its derivation but Prometheus rejected the SSH builder connection before compilation. The Lojix remote-only `.#checks.x86_64-linux.test` build also reached its dependency derivations, then failed before compilation because the same Prometheus SSH builder connection could not start. Both used `--max-jobs 0` and fallback disabled. These checks do not prove Rust compilation, VM execution, or actual Lojix materialization.
...
At this checkpoint the recorded HM panes for Field `03e825` (`wQ:p9`) and retained `6db4fe` (`wK:p2`) both returned `pane_not_found` on direct Herdr read. No delivery to either route is claimed. The published report and source branches remain the handoff evidence until an exact live route is rebound or read.
