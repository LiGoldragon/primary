...
The direct predecessor handoff is `flows/9ddcbc/refresh-handoff.md`, Primary revision `e5e0938`. Its current sequence governs this recovery: do not resume 9ddcbc implementation; preserve 9ddcbc through successor startup, identity, route, title, handoff, and target-read receipts; launch only through the corrected deployed Flow Nexus on the next Codex server; do not bypass Flow manually. Before this Field Sol launch, receive the tested Mind adapter and Home-patched Herdr revisions; `eb7bae` alone deploys one coherent package graph and witnesses installed/source parity plus service/socket health; Field Low `df09b6` runs exactly one disposable GPT-6 Luna medium Flow Start and verifies the next wrapper/socket/model, one-block startup, Flow ID, HM/title, native turn, and receipt. Existing mains remain untouched. Only after that acceptance may this Field Sol launch proceed. Refresh remaining Codex mains one at a time after this successor is ready, then Claude mains; never bulk retire.
...
Launch gates: the exact `gpt-6-sol` endpoint is coherent across client, home, control socket, Herdr executable selection, and transcript root; the launcher can inject this one prompt and read native acceptance; the model/title map is exact; all selected source hashes still match; and the Flow request is accepted without invoking or mutating a predecessor. Any failed gate remains a reported blocker, not a reason to launch around Flow or start a competing controller.
