...
# Successor reference archive — moved by primary 840e42’s v4 ruling

The following are attributed source records, historical instructions, and reports. Preserve their provenance. Current successor boundaries in the first user prompt govern action when an older embedded instruction conflicts. This relocation does not constitute new native skill injection or new human-turn delivery.
...
2026-09-16 Successor package v3 review (subflow, package commit 6616c9411): spacing fixed; the nine cross-lane topics whole; embedded log and cf7879 report current at assembly; boundary complete but for the four turns' hashes and a softer wording of the per-wake Haiku checkup and per-proof Fable audit; the dispatch still a field list, rightly, since the user prompt at 232,574 bytes exceeds two limits the review witnessed in the daemon binary: a dispatch record is rejected over 262,144 bytes (the daemon renames it into dispatch/rejected; this session's own record, 285,077 bytes, was so rejected on the file-drop path while the control-socket dispatch carried it), and Linux caps one argv string at 128 KiB; no user-prompt-file flag exists, only --append-system-prompt-file. Ruled: v4 moves the bulk (the lane log, reports, cf7879's report, the ten turns, the nine decisions, the raw vision) into the appended system prompt file and keeps the argv user prompt to the boundary block, the items block and pointers, under 100 KB, the record under 256 KiB, dispatched by the daemon's control-socket op with the file drop as fallback; the two boundary nits fixed. Conditional launch word given: when v4 is pushed and Codex's own byte checks pass, launch through the daemon dispatch, run the three post-launch checks, and have the successor report paired to this flow by codex queue and cross-session message; this flow's recycle signal follows the paired report.

2026-09-16 The v4 order and conditional launch word queued to Codex (01a0a931). The harness's classifier denied this flow's own ScheduleWakeup call once; retried. If the loop cannot re-arm, the report-file watch and subflow notifications remain the wake signals.
...
Context: said to the primary Claude 840e42 right after the cloud-component order, completing the sentence that broke off. "We should do that first" is also a working instruction, recorded in log.md. Logged directly by the main flow before acting.
...
Context: said to the primary Claude 840e42 after it reported that cross-session messages sit held for approval; "hooks like that" refers to those delivery notices. "Let's hook up flow" is also a working instruction, recorded in log.md. Logged directly by the main flow before acting.
...
Context: said to the primary Claude 840e42 at the end of the same message, on how a flow talks to Codex and the components without a flawed interface. Logged directly by the main flow before acting.

> Basically, I was saying we could maybe use an MCP that takes a single string. It's just a special MCP bridge, like a home MCP or persona MCP, that talks basically persona, which is, I guess, the whole language. You would have the orchestrate or message, and the variant would be the payload.
...
Context: typed to the primary Claude 840e42 in the same message as the channel statement. "Menchie" is read as Mentci (the name Vision/ethos.md carries) and "CreoOS" as CriomOS, both corrected in the quote; "Unity" is left as typed, not recognized, asked in the reply. "Let's decide what that is" is a working instruction, recorded in log.md. Logged directly by the main flow before acting.

> Yeah, fuck it. Let's just make a Mentci app that basically embeds a very minimal CriomOS. Let's decide what that is and just what we need to run the persona system minimally, I guess, because you're going to be running on a mobile, but you can have some small amount of flows from the phone if you need to. It's just that the phone agent is going to be thin because you don't want it. I guess it's asynchronous. It would run a job, I guess, so that it can keep the client alive on Android. It might keep the phone awake to run AI, so maybe it's just a thin client. It sets up a server, and it could potentially even have a mode where you can run a small local server to maybe debug stuff.
>
...
Context: said to the primary Claude 840e42 in the same message as the web-access statement, thinking through how datom names a web app mapping; the questions ("how do we call it?") are answered in the reply from Vision/ethos.md's import form. Logged directly by the main flow before acting.
...
Context: typed to the primary Claude 840e42 mid-turn, answering its recommendation of email with a chime. The questions (is Matrix too heavy, is there a better system) are answered in the reply, unwitnessed. Logged directly by the main flow before acting.
...
Context: typed to the primary Claude 840e42 as the living went to bed on 2026-09-16 (local evening of the 15th). "I'm going to bed now. Let's use it." and "Maybe you want to refresh" are working instructions, recorded in log.md. Logged directly by the main flow before acting.

> Design and strategy by Codex is used to do a lot of implementation of ideas through the night so that we have stuff to test in the morning. I'm going to bed now. Let's use it.
>
...
Context: said to the primary Claude 840e42 in the same message as the rating statement, continuing it. "Let's look at the Latin, the Sanskrit, and the Spanish" is also a working instruction, recorded in log.md. Logged directly by the main flow before acting.

> which is why, essentially, every living person has its own persona, its own machine. It's a machine person. Machina persona, right? Persona is short for that. It's a thinking machine.
>
...
Context: typed to the primary Claude 840e42 in the same message as the work-estimation statement. "lint app" is read as the Slint app of the mobile statement; "criome" as CriomOS, both left as typed. Logged directly by the main flow before acting.
...
Context: said to the primary Claude 840e42 in its first message to this flow, on finding the new flow after the succession notice. "Let's get this machine rolling", "let's use it" are working instructions, recorded in log.md. The self-corrected sentences on how the reset works are kept as said. Logged directly by the main flow before acting.
...
## The 5-hour window is the gauge for fair usage of each; overused Fable falls back to Opus; slack is counted internally so the system manages usage and knows when to start flows, waiting on design when Codex usage is high, using overabundant Codex to think out loud with its own subagents; the interface must be flawless

Context: answer to this flow's first anatomy question (what counts as slack). "Cloe" is speech-to-text for Claude, corrected in the quote. "4, so it's uploading onto codex" left as transcribed; "offloading" is the likely word, asked in the reply. Logged directly by the main flow before acting.
...
Context: said to the primary Claude 840e42 in the same message as the quota answer. "Nick's way" is left as transcribed, not recognized; asked in the reply. Logged directly by the main flow before acting.
...
Context: said to the primary Claude 840e42 in the same message as the quota and testing statements; the living stopped mid-message on seeing the succession notice, then resumed to this flow. Logged directly by the main flow before acting.

> The deciding point at which enough testing means we suggest deployment is to come back on the point of how to contact Psyche. It was a report documenting:
>
> * what's been put together for what
> * what we've implemented
> * what it looks like
> * how reliable it looks
> * what the tests that it's passed look like, as detailed or undetailed as warranted
>
...
Context: said to the primary Claude 840e42 while the relay of its previous words was out. "Get a Codex secondary to build a working cloud component with a sensible anatomy" is a working instruction, recorded in log.md; the sentence broke off ("based on our best design of...") and the living asked to be told what they were talking about, answered in the reply. Logged directly by the main flow before acting.

> We need to separate the private, sold part of primary and the data. Make everything about primary universal, so there could be a few repositories involved in making a primary and making a persona. Basically, you use your GitHub namespace for storing all your data, and we should host our own Git server, to be honest. That's probably the smartest thing to do right now because we have all the repositories anyway, and we have a domain name, so it's taking control with the cloud component.

-- psyche, STT.

## An equivalent repository on our Git server for each web app mapping, editable by proposal; a changed web app goes through the Psyche for approval or review, over whatever our channel is

Context: said to the primary Claude 840e42 at the end of the same message as the web-access statement. The closing question, what the channel to the living will be, is answered in the reply. Logged directly by the main flow before acting.
...
Context: typed to the primary Claude 840e42 in the same message as the cloud-order statement. "before IP" is left as typed, read as the previously seen IP; whether Tailnet means Tailscale's tailnet or our own mesh is asked in the reply. Logged directly by the main flow before acting.
...
Context: said to the primary Claude 840e42 in the same message as the quota statement. "We have to also start hunting down" is also a working instruction, recorded in log.md. Logged directly by the main flow before acting.

> We would keep tests that have been reliably, thoroughly reviewed for being reliable in real-world tests, not just fake tests.
>
...
Context: said to the primary Claude 840e42 in the same message as the Cloudflare statement. "TerraShady" and "Luna model" are left as transcribed, not recognized; asked in the reply. "Let's get that web browser access working" is also a working instruction, recorded in log.md. Logged directly by the main flow before acting.
...
Context: typed to the primary Claude 840e42 after it reported the deploy blockers and the channel question. "Astra medium" is left as typed, not recognized as a model name; asked in the reply. The numbered lines are a working instruction as well, recorded in log.md. Logged directly by the main flow before acting.
