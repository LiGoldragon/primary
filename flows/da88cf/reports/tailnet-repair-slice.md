...
|---|---|---|---|---|
| horizon-rs | `tailnet-repair-da88cf` (and `main`, on the main flow's ruling) | `a3ddaf8685b920093a2328b85ba350a04e11477a` | `ee8d6f8d` (0.12.0) | 0.13.0 |
| goldragon | `tailnet-repair-da88cf` | `a3fdb232d4fd3d6ef752f64e85546749a1f87b28` | main `8c4d03de` | data plus Horizon pin |
| CriomOS | `tailnet-repair-da88cf` | `216b403524400c82e7d5fd30ab980946a9425bc8` | main `d193bafc` | no version surface |
...
**goldragon** repins horizon to 0.13.0. On the main flow's later order it also sets Prometheus to `NixBuilder.Some.8` with its Metal core count corrected from 8 to 16, and removes ouranos's `NixBuilder`. It declares the tailnet references below and `country MX` on Prometheus's router. It gives ouranos `UsbDownlink.{ 10.44.0.0/24 }`. Prometheus gets **no** UsbDownlink, because `10.18.0.0/24` is its Router LAN (`CriomOS-lib` `network.lan.subnet`), and a second network with the same CIDR would collide. The controller's CA is `None` on the branch until step 3.6.
...
All builds ran on Prometheus. Evaluation ran on ouranos, with `max-jobs = 0`, `builders = @/etc/nix/machines` and `--option fallback false`. Every remote build line reads `building '…' on 'ssh-ng://nix-ssh@prometheus.goldragon.criome'`.

| Check | Result | Evidence line |
|---|---|---|
| horizon-rs `checks.default` (cargo test, 10 contract tests incl. `tailnet_roles_carry_trust_anchor_and_secret_references_to_every_member`, `router_country_and_usb_downlink_project_for_consumers`) | pass | `building '/nix/store/sck9fnaf…-horizon-test-0.13.0.drv' on 'ssh-ng://nix-ssh@prometheus.goldragon.criome'` |
| horizon-rs `no-free-functions`, `no-inherent-methods` | pass | Nix builds rc 0 (the scripts also passed run directly on the source before the push) |
| horizon-rs generated code | regenerated by the pinned Ethos generator in a Prometheus build; `build.rs` byte-equality holds | `horizon-regen` build copied from `ssh-ng://nix-ssh@prometheus…` |
| goldragon `nix flake check` (compose with 0.13.0; ouranos/prometheus projections; new jq assertions: MX, tailnet references, UsbDownlink, builder set = prometheus at 8) | pass | `building '…-horizon-definition.drv' on 'ssh-ng://nix-ssh@prometheus.goldragon.criome'` |
| CriomOS `checks/tailnet-declaration` | pass | `building '/nix/store/y68gfk8r…-tailnet-declaration.drv' on 'ssh-ng://nix-ssh@prometheus.goldragon.criome'` |
| CriomOS `checks/tailnet-enrollment` (VM test) | pass, script 35 s | `building '/nix/store/c507wd1f…-vm-test-run-tailnet-enrollment.drv' on 'ssh-ng://nix-ssh@prometheus.goldragon.criome'` |
| CriomOS `checks/resolver-role-policy` | fails at evaluation, **pre-existing**: `attribute 'wireguardPublicKey' missing` from its fixture's `network` override. It fails identically on CriomOS main `3e2cc8be`. The tailnet fixture migration inside it is untested for that reason. | evaluation error |

The VM test witnessed these steps:
1. Client `openssl s_client … -verify_return_error -verify_hostname controller.fixture.criome` succeeds against the system store. This covers CA trust, the name constraint and the SAN.
2. `headscale preauthkeys create --user 1 --reusable --expiration 1h` prints exactly one token.
3. Both nodes log `tailnet-enroll: backend state NeedsLogin; enrolling with https://controller.fixture.criome:8443`. The first attempt uses the placeholder key and fails. The unit retries, and after the real key lands both reach `Running` and see each other as peers.
4. `ControlURL` is the declared one.
5. A re-run logs `backend state Running; already enrolled, not re-registering`, and Headscale's node-ID set is unchanged (2 nodes).

The whole-OS evaluation of the real goldragon projection through CriomOS was **not** run. The running Lojix cannot yet materialize a 0.13.0 horizon input (§2).

**Builder capacity after deploy (I, from horizon 0.13.0 projection plus CriomOS `nix/client.nix` and `nix/builder.nix`):**
- Prometheus `max-jobs = 8`, `cores = 8`. It is a dedicated builder (Center plus remote builder), so it takes `node.maxJobs`/`buildCores`, which both equal the NixBuilder's 8.
- Ouranos has no NixBuilder, so its local `max-jobs = 0` (declared, replacing the user nix.conf hotfix). Its `/etc/nix/machines` holds the one line `ssh-ng://nix-ssh@prometheus.goldragon.criome x86_64-linux /etc/ssh/ssh_host_ed25519_key 8 10 big-parallel,kvm,nixos-test - <prometheus host key>`.
- Tiger stays out of the list because it is `online False`.
- goldragon's check witnesses ouranos's projected `builderConfigs` = [prometheus, maxJobs 8] (**W**).
...
- CriomOS main moved to `3e2cc8be` after this branch's base `d193bafc`. The two changes overlap only in `flake.nix` (check list), so expect a trivial merge.
