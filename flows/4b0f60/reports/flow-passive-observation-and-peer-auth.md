...
The passive Field overview at [`tools/field-census.mjs`](/home/li/primary/tools/field-census.mjs) read-only joins HM and Herdr exact five-tuples; [`codex-context.mjs`](/home/li/primary/tools/field-census/codex-context.mjs:198) validates native UUID against metadata/transcript path and [`claude-context.mjs`](/home/li/primary/tools/field-census/claude-context.mjs:100) validates session IDs. A direct overview at 21:07:55Z reported 19 Herdr records, 16 exact registered routes, 3 unmatched Herdr rows, 11 unmatched registrations, and native context observed for 15 rows; task and availability remained unknown. It is a current read-only `--overview` observation, not the proposed Flow typed CLI. Root's 217k/258400 last-input proxy (84%) and 9dd's 207k proxy (80%) are neither native used/usable counters nor refresh-threshold proof. An old per-row rollout quota is not current account quota: current account is attributed at 42%, while old rows showed 90/97%. These are observation checks, never sender auth or availability authority. No socket query, fetch, or build was run for this audit.
...
- **Reported local evidence:** Low832b08ba standalone remote-gate test; Flow `98f68df` binary SHA without source map; Message 0.12.0 parity unverified; restored census `f26ac801e` 11/12 mapping without duty proof; local kernel 7.1.8 without feature probe; and the 21:07:55Z passive overview metrics/limits. No coherent tested pair is returned.
