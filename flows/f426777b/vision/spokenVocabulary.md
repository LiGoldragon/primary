...
## 2026-08-26 — a different vocabulary one abstraction up from Rust; "trait" disliked as acoustically ambiguous; research ontology and category theory; better terms for Ethos and a more specific trait declaration

(The psyche's own transcription of their audio statement, typed after
ruling that a machine transcript would land in the wrong stratum. The
statement also rules on nexus-document syntax and trait declarations;
those passages are additionally logged in nexusTraits.md.)
...
Context (agent-authored, separate from the psyche's words): continues
aa4c7747's spokenVocabulary ground (a spoken vocabulary for software
engineering; favor the terms that come back a lot and stick) with a
new criterion made explicit by example: acoustic unambiguity (the
Rust/REST confusion). Research directed: ontology, category theory,
