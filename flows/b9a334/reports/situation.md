# ProtoformStack situation

Flow b9a334, a design flow, 2026-09-04. Remembering flow 6329f1 at depth one, reading flow ad19b1, and collecting every psyche record on the topics 6329f1 touches. Written by the main flow from what four read-only subflows returned; each claim carries who found it and whether it was witnessed (looked at) or relayed as a claim (read in a record, not checked).

## The situation in one paragraph

Flow 6329f1 realized and deployed the ProtoformStack train on 2026-09-04 between 03:08 and 17:14, and on the living's fifth word landed the Protos vision and the Ethos Declaration in one sweep, half-read. Flow ad19b1, a design flow open across the same day, closed at 21:06 on the living's typed ruling that the key-value delimiter and the map concept are dropped entirely from protos and its dialects. The ruling came about four hours after 6329f1's landing. Everything 6329f1 landed and built therefore carries the map: Vision/datom.md's Syntax, the Declaration's associated constants and map types, Vision/protos.md's delimiter list, datomic's Map variant with its guillemet reader and writer, and protos's Guillemets enclosure. Neither flow erred. The later ruling has not yet been carried anywhere: not into the distilled vision, not into the code, not into the skills.

## What 6329f1 did

Realization flow, Claude Fable 5.1, opened 03:08, landed 17:14 (git author dates, witnessed by the gathering subflow). Its brief, typed while the living went to sleep: remember 1c282d and ad19b1, rewrite datom and ethos-zero, build a new POC stack on a multi-repo branch-train with orchestrate as the MVP, its CLIs speaking datom; decide alone, mine psyche when unsure.

- **The train, three waves, on main and deployed** (claimed by 6329f1's log and last reply; versions not re-witnessed by this flow): protos 0.15.1, datomic 0.9.1, ethos-zero 1.3.1, signal-orchestrate 0.20.1, meta-signal-orchestrate 0.14.1, orchestrate 0.29.2 as the live nexus since 15:19 CEST, claude-answers 0.5.1, curriculum-deploy 0.5.1. Three Lojix deployments (156, 160, 164). Three independent witnesses of the CLI replies as canonical datom. A release-train record at release-trains/ProtoformStack.datom (witnessed present).
- **Skills landed**: the orchestrate skill with curly-quoted reasons; new protos, datom and ethos skills at Curriculum 67459dec, approved by the living on the word "keep going" without a line-by-line read (6329f1's own statement).
- **Its Design carried the map.** Written while the living slept, the Design declared `Datom.Map` rendered as `« k v »`, an ethos map type `Roles.« Text Integer »`, associated constants `« CAPACITY Integer »`, and a protos `Situation.« Path Extent »`. It cited ad19b1's earlier ethos record, "it should be CAPACITY. and use a key-value map delimiter", which was the only ruling on the map when 6329f1 read ad19b1 at 03:08 (6329f1's remember-ad19b1 report carries that record at its line 27 and no drop; witnessed).
- **The fifth word.** The living read the two distillation proposals up to the Declaration's File section, spoke five protos corrections, one vocabulary correction and two ethos corrections (all eight verbatim in the appendix under Protos, Vocabulary and Ethos, flow 6329f1), then ruled a one-sweep landing: apply the corrections, land, log the raw psyche, summarize. Landed at commit 23aaf7a7f: Vision/protos.md replaced whole (95 lines), a Declaration section appended to Vision/ethos.md (285 lines), Vision/sources written, the three raw records archived. All witnessed present by the remembering subflow.
- **Afterwards**: flow e996e8 remembered 6329f1, gathered the undistilled candidates on protos, ethos and vocabulary, presented them to the living, and landed nothing (e996e8's log, witnessed).

## What ad19b1 settled

Design flow, closed at 21:06 (commit 55554e368, witnessed). It remembered e4a40e and continued the distillation.

- **Landed during the flow** (witnessed in the tree): datom Meaning into Vision/datom.md (5c5f75977): no MeaningOrString, strings are strings and Meaning is Meaning, Meaning is datom. Kinds Identity into Vision/ethos.md (c2ac4cdb2) after the ruling "kind is an ethos concept, narrower than ethos; it goes in ethos": a kind is identified as a Rust trait is, by its name and its constraints; Vision/kinds.md folded away.
- **Researched**: the merit of the key-value map as against a vector of structs (reports/mapMerit.md) and the uses of the key-value paradigm across systems (reports/keyValueUses.md), which concludes that most things called key-value are not maps. One witness: Rust trait items compile in any order, only cycles refused.
- **The ruling, typed, logged in flows/ad19b1/vision/protos.md:**

> ok lets drop that delimiter and concept entirely from protos and its dialects.

- **Spoken beside it** (flows/ad19b1/vision/datom.md): "no, its all bad. your using a map that should be a struct"; "Datom is a revolutionary approach to data representation. It doesn't need to implement something simply because it has been so standard in the past."
- **Ruled, not landed by ad19b1** (its last reply, relayed; the states witnessed by this flow's subflows): the map drop; the Declaration, whose associated constants "can no longer be a map", with the bracket form `[ CAPACITY.Integer ]` named as the candidate; three skill lines awaiting the living's wording: spirit, "never carry what you have not understood"; psyche-distillation, "a proposal decides, it never marks a part of itself undecided"; psyche-interraction, reading a transcription for the known confusions, Rust heard as rest among them.
- **On distillation practice** (flows/ad19b1/vision/distillation.md, verbatim in the appendix): a proposal that says "not yet designed" decides ahead of time that it cannot be approved.

## Where the ruling and the landed state disagree

Every place below was witnessed by the gathering subflow at the line or revision given. None has been changed by this flow.

| Place | What it carries | Where |
|---|---|---|
| Vision/datom.md, Syntax | the map in prose; two examples written with « » | lines 113-116; 148-152 |
| Vision/ethos.md, Declaration | a map type; associated constants in the map delimiter | lines 156-157, 167, 174, 236, 242 |
| Vision/protos.md | guillemets named among the delimiters | line 45 |
| datomic, main 4712361c (0.9.1) | `Map(Vec<Pair>)` on Datom; a guillemet reader and writer | src |
| protos, origin/main 48061367 (0.15.1) | the Guillemets enclosure with its glyphs | src |
| ethos-zero 1.3.1 and the generated Rust | map types to BTreeMap; « CAPACITY » to associated consts | claimed from 6329f1's Design, not witnessed |
| the datom and ethos skills, Curriculum 67459dec | whether they show « » | not checked by this flow |
| Vision/datom.md's "map of Text to Integer" example | holds `Ada`, not an integer (6329f1's writer flagged it) | the same lines, 148-152 |

The eight psyche records on the map trace one arc (gathering subflow, all quoted in the appendix under Datom): `Map.[key.val]` on 2026-08-11, guillemets on 2026-08-26, the drop on 2026-09-04. No record argues for keeping the map.

## What the living has not reviewed

From 6329f1's last reply, relayed; the texts are in place for the reread the living said it would do.

- Vision/protos.md past the Direction paragraph, whole.
- The Declaration in Vision/ethos.md from its Imports section on.
- 6329f1's own design decisions, never ruled: Situation and Path beside the Protoform; the Qualified structure for `Vector<Text>`; `Corporal<C>` with a type parameter; the wire `Frame.{ Version Body }` with Refusal and no contract id; the version as wire version; aliases in Signal roots; Copy for unit-only enums; boxing recursion; the compile-time assertion as the association's Rust; `Meaning.[ Plain.Text ]`; `True` and `False` as booleans; the bare-string rule for headed chains in a Text position.
- The three dialect skills' wording.
- Orchestrate's documentation, the ClientFailure shape, the CriomOS and CriomOS-home pins.

## What is hanging

- **Code lags the landed vision** (6329f1's last reply): the crates still say `Structural::delineate`, `Conceptual<C>`, `Embodied`, and the Library and Signal roots; the vision now says Protosizable on text, Conceivable, Sized, and the variant-headed ethos with kinds, types, signal and sema. No record names the kind whose capability is incorporate.
- **The ethos variants**: field order, and whether each variant carries imports, are unstated; where a hand-written association lives once kinds and types are separate variants is unsaid.
- **The backslash escape inside parentheses**: the protos writer's own choice, read by protos main today, left undesigned by the vision (6329f1's log, witnessed by its read-trivial subflow).
- **Lojix and the frozen nexuses** (upgrade, terminal, spirit) still speak Dotos and schema-rust; beads orchestrate-dt4 and the Lojix bead track them (claimed).
- **From ad19b1**: the three skill lines awaiting wording.
- **From e996e8** (relayed): 04db2fd2's directionAsymmetry record was approved for distilled vision and is carried by Direction but is neither in Vision/sources/protos.md nor archived.
- **From the gathering**: fourteen contradictions or tensions between the latest raw record on a subject and the landed Vision text, listed apart in the appendix; none resolved by this flow.
- **Bookkeeping**: no ProtoformStack branch is visible locally in any repository though 6329f1's log says branches were kept; its eight worktrees under /home/li/wt still exist (witnessed).

## Forks the living is owed

Framed, not decided. Each names the candidates the records already hold.

1. **Carrying the drop into the distilled vision.** Vision/datom.md's Syntax block, the Declaration's constants and map type, and Vision/protos.md line 45 all need re-distilling. One sweep as on 6329f1's fifth word, or section by section with the living reading each.
2. **The form of associated constants.** ad19b1 named `[ CAPACITY.Integer ]` as the candidate once the map is gone. The living also said the key-value delimiter "only works if their order doesn't matter" and that key-value delimiters were for sections that cannot have the same key, so a bracket carries an order the constants may not need.
3. **What map-typed data becomes.** `Roles.« Text Integer »` and its BTreeMap have no form. The research direction was that a map is a vector of structs; whether ethos declares such a vector, and whether datom keeps any pairing concept, is unruled.
4. **The code.** A fourth wave of the train removing Map and Guillemets from protos, datomic and ethos-zero, regenerating the signal crates, orchestrate, claude-answers and curriculum-deploy, and redeploying; now, or after the re-distilled vision is read.
5. **The half-read landing.** Whether to review Vision/protos.md and the Declaration now, before the drop is carried into them, or to carry the drop first and read once.

## Sources

- flows/6329f1/vision/archive-ethos.md, archive-protos.md, archive-vocabulary.md; flows/6329f1/log.md; flows/6329f1/reports/remember-ad19b1.md; the 6329f1 transcript's final assistant message (remembering subflow).
- flows/ad19b1/vision/*.md (eight files); flows/ad19b1/log.md; flows/ad19b1/reports/mapMerit.md, keyValueUses.md; flows/ad19b1/witnesses/traitItemOrder.md; the ad19b1 transcript's final assistant message (ad19b1 subflow).
- flows/e996e8/log.md; flows/index.md (remembering subflow).
- Vision/datom.md, Vision/protos.md, Vision/ethos.md, Vision/sources/*.md; git log for 55554e368, 7d19dcacf, 23aaf7a7f; datomic main 4712361c; protos origin/main 48061367 (gathering subflow).
- flows/62022e8f/vision/designPractice.md; flows/01a0428b/vision/useASubflowToPutTheReportTogether.md; flows/04db2fd2/vision/artifacts.md; the flow-evidence, main-flow and subflow skills (approach subflow).

## Appendix — every psyche record gathered
