# Realization launch prompt

For the living to paste into a Codex session started in the primary
workspace. Everything the receiving flow needs is in the block: the
skills typed as `$name` on the first line so they load at the right
stratum, the decisions and the outcome, the whole of the landed Vision
and Intent verbatim so it arrives as typed prompt text rather than as a
link, the direct paths, the transcript invitation, and the beads.

````````
$spirit $subflow $realization $psyche $psyche-acquisition $psyche-interraction $behavior $vocabulary $protos $datom $ethos $nexus $lojix $edit-coordination $feature-development $repository-lifecycle $versioning $testing $file-editing $nix-workflow $orchestrate $flow-evidence
...
Realize the Vision and Intent below in code. On 2026-09-09 the living
approved the whole distillation proposal of flow 564f55 ("Okay, then the
proposal is good, and land it all"), and it was landed into the distilled
records reproduced verbatim in this prompt. Those records are the
authority for what to build. This flow builds it.

The repositories that change:

  /git/github.com/LiGoldragon/protos              the protosic layer, the shared style
  /git/github.com/LiGoldragon/datom-codec         the datomic layer and the derive
  /git/github.com/LiGoldragon/ethos-zero          the generator and the daemon
  /git/github.com/LiGoldragon/signal-ethos-zero   its signal contract
  a datom-codec derive crate, which does not exist yet: datom-codec is a
  single package today, not a workspace.

Decisions already made, which the code honours and which are not reopened:

- String, not text, everywhere.
- The error example carries no parentheses: `Value.x`, not `Value.(x)`.
- Every declared struct and enum bears both kinds; aliases inherit them
  through the type they name and carry no derive.
- The datom kinds in a generated signal library are conditional on a
  `datom` feature that the CLI and client enable and the Nexus does not;
  the Nexus compiles without datom-codec as a dependency at all.
- No migration path is written. Backward compatibility is not a design
  variable: every consumer is updated.
- Meaning is postponed — a parenthesized text lands as a plain `String`
  today, with the later type marked in code.

Land it on main with the gates green, versions bumped as the versioning
skill rules, and consumers re-pinned. Write the report into
FLOW_DIRECTORY/reports/.

# The psyche, verbatim

What follows is the distilled Vision and Intent as it stands, reproduced
whole. Read it as the ruling text, not as a summary of one.
...
The distilled records above, in the primary workspace:
...
Where each statement came from, and who approved it:
  ...
  /home/li/primary/Intent/sources/anatomy.md
  /home/li/primary/Intent/sources/conversion.md
  /home/li/primary/Intent/sources/context.md

The lane this flow continues, flow 564f55:

  /home/li/primary/flows/564f55/log.md
  /home/li/primary/flows/564f55/reports/landing.md    what was landed, superseded and destroyed
  /home/li/primary/flows/564f55/reports/vocabulary.md
  /home/li/primary/flows/564f55/reports/codexLaunch.md
  /home/li/primary/flows/564f55/vision/               what stayed raw after the distillation
  /home/li/primary/flows/564f55/notion/datom.md       a notion: it rules nothing

Raw records that were left raw on purpose, and still carry unruled
thinking on these subjects, are found under /home/li/primary/flows/*/vision/
and /home/li/primary/vision-raw/.

# The transcripts

Be generous in understanding as much as you can from the transcripts,
weighting by recency: the most recent sessions on these subjects carry
the living's latest thinking, including what no record has caught up
with yet. `transcript search <pattern> --recent <n>` finds typed
messages across recent transcripts with file and line; `transcript show
<session>` prints every typed message of a session with its line number;
`transcript raw <session> <lines>` prints the records at those lines.
Start from the typed messages, widen to model text with `--assistant`
only when the typed words are not enough, and never read a transcript
whole. Quote with the transcript and the line number. Flow 564f55's own
session is https://claude.ai/code/session_0199mTWBDkRm53xFVmZuMFqp.

# The beads

All in the primary repository (/home/li/primary); protos, datom-codec,
ethos-zero and signal-ethos-zero carry no beads database of their own.

  primary-xqb       Protos engine rebuilt on 2026-08-04 syntax and encoded-type rulings
  primary-xqb.4     Traits-ontology standing directive: soft requirement with per-repo migration files
  primary-xqb.5     Projection profiles: token-lean canonical and pretty-printed human projections
  primary-xqb.6     Tree-sitter grammar updates for dotos and ethos
  primary-xqb.8     Shortcut engine up: Ethos in, generated Rust out for Signal, Nexus, Sema
  primary-xqb.8.3   Universal signal repo carries the router enum and the handshake payload
  primary-xqb.8.4   Phase 1: psyche-component Ethos fixture and exact expected Rust shape, psyche-reviewed
  primary-xqb.8.5   The Meaning structured string has a ruled shape and annotation vocabulary
  primary-xqb.8.8   Repository manifest describes the actual three-stack estate without relic canonical entries
  primary-xqb.8.9   Protos shared structural-transcoding boundary is reconciled with the approved parsing Intent
  primary-xqb.8.10  Superseded numeric signal-contract allocation registry is retired from Protos in favor of the ruled universal signal surface
  primary-xqb.8.11  Production components migrate from legacy and frozen notation dependencies onto the correct-new stack
  primary-xqb.8.12  Psyche component supersedes the separate spirit-ethos source repository
  primary-xqb.8.15  Implementation round reviewed and audited by the Designer against ruled ground
  primary-hqu       Core crates shed superseded generations: one Ethos reader, one lowering path, ruled syntax in every live fixture
  primary-eyr       Protos train unified on main; new stack live end to end
  primary-06m       Reacquire protos vision from the basics: structural parsing
  primary-bfs       Ethos and Dotos express general data anatomy outside component roles
  primary-83s       Rename NOTA to Dotos with the new protos engine train landing
  primary-7r4       Migrate broader public Text aliases to String
  primary-43a       Preserve trait pickups for generic Struct and Enum Logos declarations
  primary-5cz       “ ” and « » are easy to type in the editing environment
  primary-qdn       Ethos carries Nomos transformer input schemas (daemon-chain type mirroring)
  primary-vq6.8     Spirit lands as the first protos-based component
  primary-vq6.9     signal-mind contract crate generated from interface.ethos
  primary-vq6.10    Orchestrator and messenger contract crates switch to Ethos
  primary-pjm       Slice 2: minimal Ethos file kind proving the file-kinds law (blocked)
````````

## Sources

- `/home/user/primary/flows/0d557b/vision/launchPrompt.md` — the living's
  ruling on how this prompt is built, dictated 2026-09-09: the fresh
  Vision goes in the prompt, the direct links to the distilled Vision,
  the invitation to read transcripts generously by recency, and the
  skills typed so they land at the right stratum.
- `/home/user/primary/flows/564f55/reports/landing.md` — what was landed,
  and the amendments the living made to the approved proposal.
- `/home/user/primary/flows/564f55/reports/codexLaunch.md` §6 — the draft
  prompt, its skill list verified present, the repository paths and
  versions witnessed on the living's machine, and the correction that
  `Vision/signal.md` did not exist at the time (it does now).
- `/home/user/primary/flows/564f55/log.md` — the approval quote and the
  recorded next step, "a Codex flow for realization".
- `/home/user/primary/Vision/{protos,datom,ethos,signal,sema,nexus,flowNexus}.md`
  and `/home/user/primary/Intent/{anatomy,conversion,context,data,mandatoryTraits,protosParsing}.md`
  — reproduced verbatim by concatenation, not retyped.
- `/home/user/primary/.claude/skills/transcript-search/SKILL.md` — read as
  evidence for the transcript mechanism named in the prompt.
- `/home/user/primary/.beads/export.jsonl` — the open, in-progress and
  blocked beads on these subjects; the four repositories hold no `.beads`
  directory.
