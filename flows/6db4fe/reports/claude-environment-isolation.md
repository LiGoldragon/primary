...
The changed batch fixture executes the generated POSIX shell command with a deliberately contaminated parent environment, and verifies the new job path and cleared inherited identity variables. Direct-bootstrap and retired-path fixtures pass locally and in a scoped remote Nix derivation on Prometheus that also included zsh. The first repository-wide remote `native-seat-fixtures` attempt failed before the changed batch fixture because its existing `native-seat-launch.test.mjs` could not find `herdr` in that check's build closure. Its owner corrected that fixture. A second attempt reached our fixture but its test-only `zsh` executable was absent from the same closure; the test now uses available `sh` for POSIX shell behavior. The coherent third repository-wide remote run passed all native seat fixtures on Prometheus. No local Nix fallback was used.
