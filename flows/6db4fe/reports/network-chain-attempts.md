...
Attempt 10's ten report lines were authored under this report's Orchestrate lock #3926 by Flow `6db4fe`, but a concurrent shared-working-copy commit `acace3fd` by the CriomOS source worker included this path alongside `flows/753e69/reports/horizon-usb-gateway-contract.md`. `jj show --stat acace3fd` records both files, and the real remote carried that revision. The report content and evidence stand; this is a cross-scope commit inclusion, not a claim that Attempt 10 had its own scoped commit. No remote history was rewritten. Later edits to this report are restricted to this flow's own scope.
