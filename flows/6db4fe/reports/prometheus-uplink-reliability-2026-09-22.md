...
Times below are 2026-09-21/22, Mexico City UTC−06:00. No network configuration, service, profile, radio power, source pin, or VM was changed in any round. Private client identifiers and credentials are omitted.

## Presentation summary — no operator identifiers

Prometheus was serving three Wi-Fi clients at 15:39 local on Sep 21. A separate Wi-Fi audit reports that it answered over the management network at about 16:00 local; by 16:30, the known management and local paths no longer reached it. The observed loss begins in that **16:00–16:30 local (22:00–22:30 UTC)** interval. The local management journal later showed both peer sessions timing out around 16:14–16:16 local, which fits the bracket but does not identify a cause.

Ouranos's own wired Internet still works. That says nothing about Prometheus's Internet or Wi-Fi service: a public cache answered while the Prometheus cache timed out. Ouranos's USB adapter still has a physical link indication but received no new frames across the sampled interval; its Wi-Fi client repeatedly lost authentication and a fresh scan listed 14 other networks, not the Prometheus network. Those are observations from Ouranos, not a diagnosis of Prometheus. There is no demonstrated kernel or hardware defect. Power, peer operating system, radio/bridge, cable, and peer network interface remain open possibilities.

The Field already has the existing peer/console question and the living has already been asked once for the currently working network and device. This report adds **no new request**. Until that witness arrives, no peer-side repair or declarative activation is justified. The technical evidence and future configuration gates below are for operators, not presentation copy.

## Operator appendix — exact evidence and configuration

Owner: Field `6db4fe` network runtime lane. The `microvm@vm-testing` guest remains stopped under separate lock `4373`.
...
**Observed at 18:21–18:23 local.** Ouranos runs kernel `7.1.8` from its installed August generation. The exact USB adapter is ASIX `0b95:1790` on USB bus 004 port 001 at 5 Gbit/s bus speed, bound to `cdc_ncm`; its Ethernet PHY reports carrier 1 and 1000 Mbit/s. It has zero reported RX errors/drops and no new RX packets since the 16:30 snapshot, despite additional TX packets. No USB disconnect/reset or `cdc_ncm` error appeared in the bounded kernel window since 17:00. This supports a physically present local adapter and link indication **while no frames arrive on that link**; it does **not** prove that Prometheus is powered, its OS is running, or the cable/peer NIC is healthy end to end. The sampled RX/TX divergence is consistent with a peer that sends nothing, a link carrying carrier without data, or a local receive path not delivering; these readings do not distinguish them. Ouranos client Wi-Fi uses `iwlwifi`; the bounded log showed repeated authentication loss, not a firmware crash/reset signature. Nothing here supports a kernel regression or hardware defect claim.
...
**Limit and next input.** The living reports that Zeus Wi-Fi works, but its *current* SSID/address/route are not in the reachable host evidence. The Field already asked for this information once; no further ask is made here. The timed-out Zeus route is not represented as available. This round made no configuration change, so no rollback is required.

## Round 4 — local Yggdrasil visibility, without peer probes

At 18:32 local, Ouranos's Yggdrasil daemon was active/running (same process since Sep 10); `yggTun` had an IPv6 address and kernel routes to both configured peer addresses. This proves only local service and route presence. Its bounded journal records inbound/outbound TCP sessions to Zeus timing out at 16:14 and 16:16 local and sessions to Prometheus timing out at 16:16. The sessions used Ouranos's Wi-Fi link-local interface. Those timestamps fit the 16:00–16:30 loss bracket and the later Wi-Fi association failures; they do not show whether the original break was in Ouranos Wi-Fi, Prometheus radio/host, Zeus, or shared air/link conditions. The daemon's admin peer-list socket returned permission denied to this user; no current peer list or health is claimed. No privilege configuration or service was changed.
...
- Earlier AP witness: `flows/6db4fe/reports/prometheus-wifi-brief-2026-09-21.md` at 15:39 local. The roughly 16:00 Ygg ping/SSH success is attributed to Psyche High's audit text titled *Uplink Report Audit Corrections, Round Two (PsycheHigh 1b8ac0)* in its native record; the exact command receipt was not independently recovered in this round.
- Rounds 1–4 read-only Ouranos `nmcli`, `ip` route/address/neigh/link counters, NM and kernel journal, NixOS generation, sysfs driver/carrier/speed, `lsusb -t`, installed firewall unit, configured DNS/SSH, Ygg service/admin access/journal, and Tailscale state at 16:30–18:32 local. The fresh 18:21 scan's captured output contained 14 other network entries; no additional scan was run for this correction. Privileged live iptables and Ygg peer-list reads were unavailable and are not claimed.
