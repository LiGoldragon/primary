Archived on landing: distilled into Vision/protos.md (Layers) and Vision/ethos.md (Kind), flow e996e8, 2026-09-04. The content is carried there; the words are kept here.
...
Asked to rule between embody/embodied and forge/forged for the text-to-value direction, the psyche:
...
The flow had written the kind pair as Embodied beside Textual (Textual on Text). The psyche:
...
The flow had put both directions' capabilities on the embodied type. The psyche:
...
The flow had written `Lock.[ Textualizable ]` as an example embodied type. The psyche, asking about Lock:
...
The flow had asked whether `Result<Embodied Fault>` makes Embodied a kind. The psyche:
...
The flow had asked whether Embodied is the type the text embodies into, resolved per bearer. The psyche:
...
The flow had marked the yield of embody as ⟦bearer⟧, having no ruled ethos word for the type that bears the kind. The psyche, quoting the line `Embodiable.[ embody.{ [Text] [Result<⟦bearer⟧ Fault>] } ]`:
