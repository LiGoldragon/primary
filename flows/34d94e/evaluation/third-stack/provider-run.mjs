...
export async function makeProxy(upstream, secret, ca, requestFn = upstreamRequest) { let ambiguous = false; let requests = 0; const open = new Set(), active = new Set(); const server = http.createServer(async (request, response) => { if (ambiguous || request.method !== 'POST' || request.url !== '/v1/chat/completions' || ++requests > REQUEST_LIMIT) { response.writeHead(502, {'content-type': 'application/json'}); response.end(safeError()); return; } const chunks=[]; let size=0; request.on('data', b => { size += b.length; if (size > OUTPUT_LIMIT) request.destroy(); else chunks.push(b); }); request.once('end', async () => { const controller=new AbortController(); active.add(controller); try { const result = await requestFn(upstream, secret, Buffer.concat(chunks), ca, controller.signal); if(result.status < 200 || result.status >= 300) { ambiguous=true; response.writeHead(502,{'content-type':'application/json'}); response.end(safeError()); } else { response.writeHead(result.status, {'content-type': result.type}); response.end(result.body); } } catch { ambiguous = true; response.writeHead(502, {'content-type':'application/json'}); response.end(safeError()); } finally { active.delete(controller); } }); }); server.on('connection', socket => { open.add(socket); socket.once('close', () => open.delete(socket)); }); await new Promise((resolve, reject) => { server.once('error', reject); server.listen(0, '127.0.0.1', resolve); }); return {url: `http://127.0.0.1:${server.address().port}/v1`, requests: () => requests, async close() { for (const controller of active) controller.abort(); for (const socket of open) socket.destroy(); await Promise.race([new Promise(resolve => server.close(resolve)), sleep(1_000)]); }}; }
