...
const fakeBinary = path.join(dir, 'opencode'); fs.writeFileSync(fakeBinary, '#!/usr/bin/env node\nif (process.argv[2] === "--version") process.stdout.write("1.17.13\\n"); else setTimeout(() => {}, 1000);\n', {mode: 0o755});
...
function run(extra) { return new Promise(resolve => { const p = spawn(process.execPath, [driver, '--provider-descriptor', descriptor, '--secret-fd', '99', '--opencode', fakeBinary, '--output', output, ...extra], {stdio: ['ignore', 'pipe', 'pipe']}); let out = ''; p.stdout.on('data', c => out += c); p.on('close', code => resolve({code, out})); }); }
try {
