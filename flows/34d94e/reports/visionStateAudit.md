...
| Message + signal-message/meta | Durable ingress/outbox/delivery, process-derived provenance and two listeners; ordinary/meta Ethos producers. Main Message checkout `be8bfe9a01d1`. | Source behavior plus separate relay fixture's recorded Cargo/producer/meta passes. | Corrected after Fable doubt: user `message-daemon.service` observed loaded/active/running; store executable Message0.11.1. Source-revision match and functional behavior not verified. | Core meta listener returns unimplemented; relay fixture still needs main/content integration and live attachment wiring. |
...
## Fable doubt and fresh corrections

Fable correctly identified two audit omissions. A fresh user-manager check found `message-daemon.service` loaded/active/running, with fragment `/home/li/.config/systemd/user/message-daemon.service` and executable `/nix/store/qzqzf96x2siv2p02mf670vjy8qw1ppyy-message-0.11.1/bin/message-daemon`. The initial exact-name negative was accurate but insufficient coverage. Message running-state is now established; deployed source revision, active configuration and functional behavior remain distinct evidence questions.

Polling was missing from this audit. At triad-runtime revision `8dc6e30bc77c`, the synchronous `BoundMultiListenerDaemon` uses nonblocking listeners and sleeps10ms after no stream is accepted (`triad-runtime/src/daemon.rs:178`, `:307`, `:373`). Current Message source uses `AsyncMultiListenerDaemon` (`message/src/daemon.rs:55`), so the synchronous loop does not prove every daemon or the active Message executable polls. Harness revision `efa4e7c5ed5e` uses a notify watcher first, a250ms snapshot polling fallback if watcher construction fails (`harness/src/claude.rs:94`), and re-snapshots after a5s watcher timeout (`:394`). These are genuine source tensions with subscription-only observation and “Polling is forbidden” (`Vision/nexus.md:117`). The deployed paths were not traced; no runtime fix was launched.

The expected `flow` checkout being absent did not justify omitting the existing `/git/github.com/LiGoldragon/harness` implementation. It is library/tooling evidence, not by itself the complete authored Flow Nexus. This follow-up closes that discovery omission without declaring the entire runtime design implemented.

Codex challenges returned to Fable: existing `signal-message/ethos/signal.ethos:1` and `meta-signal-message/ethos/signal.ethos:1` disprove a global “nothing authored in Ethos” claim. Three active units do not by themselves establish an edge-free topology. Test definitions alone do not prove a successful build/test run. The Criome-required Lojix activation finding remains bounded to inspected sites, not proof that no path exists anywhere. These are corrections to synthesis scope, not competing implementation proposals.
