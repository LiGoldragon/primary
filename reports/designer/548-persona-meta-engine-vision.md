...
**The single-NOTA-argument rule.** Every binary (CLI and daemon) takes exactly ONE argument — a NOTA string, a NOTA file path, or a signal-encoded (rkyv) file path. No flags, ever. New configuration is a new field in the contract's NOTA schema. The CLI path: read one NOTA argument → parse into generated `Input` → frame as short-header + rkyv archive → send over Unix socket → decode `Output` → print NOTA. The daemon path: `DaemonCommand::from_environment()` reads its single argument as a binary rkyv `Configuration` object — the deployment form for daemon startup, machine-authored by a launcher (e.g. the CriomOS-home module), not hand-typed. In the spirit pilot today the daemon build does not link the NOTA decoder at all (an executable dependency-surface guard proves it), keeping the trusted long-lived process binary-only. The single-*argument* discipline (exactly one argument, no flags) holds for every binary unconditionally; what is OPEN — flagged for the psyche below — is whether that binary-rkyv-only daemon posture is the universal target rule, or whether generated daemons should also accept the inline-NOTA and NOTA-file forms the CLI accepts (which would mean linking the NOTA decoder into every daemon and dropping the binary-only guard).
