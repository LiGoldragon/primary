...
| `/git/github.com/LiGoldragon/signal-core/src/request.rs:6-19` (enum) | `pub enum SemaVerb` with twelve variants | `pub enum SignalVerb` with seven variants; doc-comments on `Atomic` and `Validate` noting the planetary-tradition rename candidates (`Bind`/`Assay`); explicit pointer to `sema-engine::ReadPlan` for the algebra | **Done** (`aa7a0d93` — seven variants, `SignalVerb` rename complete, doc-comments on `Atomic`/`Validate` carry the planetary-rename notes, `unchecked_operation` constructor in place, `signal_channel!` macro generates `signal_verb()` witnesses) |
| `/git/github.com/LiGoldragon/signal-core/src/lib.rs` `Request<P>` constructors | Free-form `Request::assert(...)`, `Request::match_records(...)`, etc. accept any payload under any root | Deprecate or rename to unchecked/internal so source scans find them; payload-first construction via per-contract `signal_verb()` mapping is the canonical path | **Done** (`aa7a0d93` — `unchecked_operation` is the named escape hatch; `RequestPayload::into_request()` is the canonical payload-first constructor) |
| `/git/github.com/LiGoldragon/signal-core/src/lib.rs` `signal_channel!` macro | No verb annotation on request-enum variants | Per `/50 §5`, accept verb annotations; generate `signal_verb()` mappings and per-variant witnesses | **Done** (`aa7a0d93` — macro emits `signal_verb()` witnesses per request variant) |
| `/git/github.com/LiGoldragon/nexus/spec/grammar.md` | Top-level Nexus records enumerate twelve verbs | Top-level records enumerate seven; algebra appears inside `Match`/`Subscribe`/`Validate` payloads as `ReadPlan` records | **Done** — "The seven root verb heads are: Assert Mutate Retract Match Subscribe Atomic Validate" + explicit "Read-algebra names ... are also not root verbs" |
...
| **Every `signal-*` contract crate with a verb mapping** (signal-persona, signal-persona-message, signal-persona-router, signal-persona-terminal, signal-persona-manager, signal-persona-mind, signal-persona-introspect, signal-persona-system, signal-persona-harness, signal-forge, signal-arca, ...) | `pub fn sema_verb(&self) -> SemaVerb` per-variant mapping returning the closed twelve | `pub fn signal_verb(&self) -> SignalVerb` per-variant mapping returning the closed seven; round-trip witness that no variant maps to a demoted name | **Partially Done** — `signal-persona`, `signal-persona-mind`, `signal-persona-message`, `signal-persona-router`, `signal-persona-system`, `signal-criome`, `signal-forge` are fully migrated (zero stale refs). **`signal-persona-introspect` (3 refs)**, **`signal-persona-terminal` (7 refs)**, **`signal-persona-harness` (5 refs)** still import `SemaVerb` from `signal-core`, which is no longer exported. These three crates currently do not compile against `signal-core@aa7a0d93`. The fix is mechanical: rename `SemaVerb` → `SignalVerb` in their `use` statements and `signal_verb()` return types. Operator-scope coordinated pickup. |
