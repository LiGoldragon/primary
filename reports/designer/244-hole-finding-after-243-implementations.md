...
1. **Fix hole 1**: extend the `Lowering` trait with
   `reply_from_lowering_rejection` and
   `reply_from_engine_rejection`. Update `Executor::execute`
   to route through them. This makes the reply-naming
   convention from `/243` §1 actually operative on the wire.
2. **Fix hole 2**: rename the macro-injected verb from
   `Observe` to `Probe` (or similar). Single-file change in
   the macro's emitter + tests.
3. **Build the publish-bridge** for hole 3: write the
   daemon-side adapter that wires `LedgerObserverSet`'s
   publish closures into `Executor`'s `ObserverChannel<Operation>`
   trait. Probably lives in a small `signal-executor-frame`
   bridge crate (signal-frame + signal-executor →
   `signal-executor-frame`). Or document the pattern
   inline in signal-executor's README.
4. **Pick the pilot as the worked example for hole 5**:
   signal-repository-ledger gets an `observable` block, a
   `Lowering` impl in the daemon, and the wire-bridge
   adapter. That commit becomes the canonical example.
5. **Macro-generate closed-enum filter defaults** for hole 4.
   Lower priority — current trusted-impl pattern is
   workable.
6. **Decide on holes 6A–6F** — most are documentation work.
