...
between per-host `clavifaber` runs, the long-lived `cluster-trust`
...
|---|---|---|---|---|
| **Publication push** — per-host clavifaber to `cluster-trust` | host → cluster | many hosts → one cluster; one-shot per host convergence | `cluster-trust` mints `Slot<PublicationCommit>` | short-lived, request/reply per submission |
| **Trust distribution** — `cluster-trust` to each host subscriber | cluster → host | one cluster → many hosts; long-lived | `cluster-trust` is source of truth; subscriber holds cursor | long-lived stream; current-state on connect, then deltas |
...
### Relation 1 — Publication push (host clavifaber to cluster-trust runtime)

A `clavifaber` convergence run on each host completes by submitting one
typed publication record to the long-lived `cluster-trust` component.
...
commit on `cluster-trust`'s side.

```
clavifaber (host probus, one-shot)
  ─ PublicationSubmission ─►  cluster-trust (Prometheus, long-lived)
  ◄─ PublicationReceipt ─    cluster-trust
```

### Relation 2 — Trust distribution (cluster-trust runtime to host subscriber)

Each host that needs to know the current trust state subscribes to
`cluster-trust`. The contract is the canonical pushed-subscription
...
host_subscriber (any persona-host)
  ─ TrustSubscription ─►        cluster-trust
  ◄─ TrustSubscriptionReceipt ─ cluster-trust  (current state snapshot)
  ◄─ TrustUpdate (event N)     cluster-trust  (each committed revision)
  ◄─ TrustUpdate (event N+1)   cluster-trust
  ...
```
...
pub struct ClusterRevision(u64);  // monotonic, cluster-trust-supplied
...
| Value | Who mints | Why |
|---|---|---|
| `Slot<PublicationCommit>` | `cluster-trust` on commit | Identity beyond content; assigned by the store, returned in reply. Clavifaber never invents it. |
| Commit time of a publication | `cluster-trust`'s transition log | Time the cluster registry committed the publication. Lives on the transition log, not on the record body. |
| `ClusterRevision` | `cluster-trust` | Monotonic source-of-truth counter; `cluster-trust` mints; subscribers receive in events. |
| `NodeName` | `clavifaber` (host hostname) | Content; the host knows its own name. Subject to validation by `cluster-trust` (NodeNameMismatch rejection). |
...
connection's auth proof, not by the field. `cluster-trust` validates
...
unix-domain socket on the cluster's central node (`cluster-trust`'s
...
machine as `cluster-trust` (development) or via SSH tunneling
...
criteria. If `cluster-trust` has filesystem access to the host's
...
| Primitive | Pros | Cons |
|---|---|---|
| Direct socket push | Same wire pattern as the rest of the sema-ecosystem; works cross-machine; immediate ack so clavifaber knows the submission committed before exiting. | Requires `cluster-trust` reachable from each host at convergence time. Failure mode: clavifaber exits 0 with publication uncommitted. |
| File-on-disk + inotify | Decouples per-host clavifaber from `cluster-trust` availability. Convergence runner exits clean even if the cluster is down. The publication file is the durable record on the host. | One-way: clavifaber gets no commit acknowledgement; `cluster-trust` may take seconds to consume. Requires file-path access (NFS/SSH/mount). |

**My recommendation**: socket push as the canonical relation-1 transport;
inotify as a fallback when `cluster-trust` is unreachable. The contract
...
both have `cluster-trust` as one endpoint; pairing them in one repo
makes the `cluster-trust` ↔ cluster-host story visible in one file.
...
- `~/primary/reports/designer/110-cluster-trust-runtime-placement.md`
  — sibling report; places the cluster-trust runtime as a new sibling
  component in the auth/security ecosystem (not inside ClaviFaber, not
  inside the current `criome` daemon, not inside Persona). Repo name
  TBD.
