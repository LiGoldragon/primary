...
0. **Uniform body parsing — Spirit 1288** (new this turn, 2026-05-31; Principle, Maximum). NOTA parsing strips the outer delimiter before handing content to the next step; the file case and the nested-inside-delimiter case converge on ONE body-parser per type. The current `NotaDocumentDecode` vs `NotaDecode` trait split is the special case to retire. Foundation work in `nota-next` derive crate: emit `from_body_objects(objects: &[Block])` per type; the two existing entry points become thin callers that strip (or don't strip) the wrapper before invoking the unified body parser. Then `schema-next` simplifies — `Asschema::from_nota_source` becomes `Document::parse(source).root_objects() → Asschema::from_body_objects(...)`; `#[nota(known_root)]` and `#[nota(name = "X")]` become general body-parsing options. Folds into designer 443 sub-agent 1 improvement #2 (nota-next derive features). Section §3 of this meta-report has the full vision.
