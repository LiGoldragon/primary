...
0a. **Schema source enum-body honesty — Spirit 1294 / 1295 — landed.** The prior bracket-body shorthand has been retired because it interleaved key-value rhythm into a vector delimiter. The honest form is now the live implementation: data variants are parenthesized records `(VariantName PayloadType)`, and unit variants are bare PascalCase symbols. The structural macro layer owns that signature shape; `schema-next`, `schema-rust-next`, and `spirit-next` have been migrated, and the assembled form remains unchanged.
