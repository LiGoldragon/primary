...
After this synthesis, the actionable backlog (designer 443 §"Recommended sequencing" + operator 265 §"Remaining Gaps" both converge here, with Spirit 1288 added as a new architectural-elegance item):

0. **Body-stream substrate — Spirit 1287 + 1290 — LANDED on main.** Operator integrated the uniform body parsing principle into `nota-next main` (`64647b1a`), `schema-next main` (`3ead0e93` + `c837c656`), with `NotaBody` as the first-class body type, `NotaBodyDecode` as the canonical semantic entry point, `CapturedValue::Body(NotaBody)` exposing macro captures as bodies (not wrappers), and four major schema built-ins (RootImports, RootNamespace, RootEnumBlock, StructFieldMap) now lowering through body streams. The original designer prototype on branch `designer-uniform-body-parser` (`38b2f74`) fed into operator's integration; the canonical reference is the main-line trio. Still open: `MacroObject::Block` paths in the macro-trait boundary — target is `SchemaMacro::lower(body: NotaBody<'_>, context) -> Result<MacroOutput, SchemaError>` once `MacroMatch` captures route into schema handlers. Section §3 of this meta-report has the full landing detail.

0a. **Schema source enum-body honesty — Spirit 1294 — pending migration.** Operator 268 §2 still shows the retired `@`-suffix shorthand inside the input/output bracket bodies (`[Record@ Entry Observe@ Query Remove@ RecordIdentifier]`). That form interleaves two objects per logical entry inside a vector delimiter, breaking vector homogeneity per Spirit 1267. The honest form: data variants as parenthesized records `(VariantName PayloadType)`, unit variants as bare PascalCase symbols (`[(Record Entry) (Observe Query) (Remove RecordIdentifier)]`). The recognized shorthand is owned by a structural macro defined in NOTA data form per Spirit 1263. Migration needed: replace data-variant pattern in `schema-next/src/macros.rs:421-446`, update schema source fixtures (`spirit-next/schema/lib.schema`, `schema-next/schemas/core.schema`), regenerate `.asschema` artifacts. Assembled-form is unchanged. Section §3 of this meta-report has the full landing detail.
...
- Spirit records: 1244 (binary daemon + feature-gated NOTA), 1246 (live asschema artifact), 1249 (rkyv discriminant stability), 1259 (strict brace key-value), 1263 (macro nodes at NOTA layer), 1267-1269 (notation honesty: no heterogeneous vectors, no meaningless wrappers, truthful representation), 1270 (three categories + module-qualified + cross-crate), 1271 (rkyv-in-SEMA + NOTA re-export through derived types), 1272 (four-object logic separation), 1274 (positional root reading), 1277 (input/output as struct fields not variants), 1278 (NOTA-layer abstraction for known-root), 1279 (two-layer structural matching + ordered patterns + no-match-as-error), 1280 (structural macros over text macros — CLOSED at schema-next 877c03f5), 1281 (NOTA as programmable syntax library for structural-macro languages), 1282 (architecture presentations use short focused graphs + multiple parts + paired with real code), 1287 (after structural parsing matches a file body or delimited object, next semantic step receives matched body's inner object stream — Maximum Correction; LANDED on main per operator 267), 1289 (operator-lane subagents are asynchronous side work; main agent keeps working — Maximum Correction), 1290 (NotaBodyDecode is the semantic decode entry point — Maximum Decision; LANDED), 1291 (schema-emitted Signal/Nexus/SEMA projections before schema-core extraction when projection slice stays small — High Decision), 1292 (schema macro handlers receive NotaBody body streams not wrapper delimiters — Maximum Decision; LANDED per operator 267 c837c656), 1294 (schema source enum-body vectors honor vector homogeneity per 1267: data variant is `(Name PayloadType)` parenthesized record, unit variant is bare PascalCase symbol; the `@`-suffix shorthand interleaving inside a bracket is retired; recognized shorthand owned by a structural macro defined in NOTA data per 1263 — Maximum Decision; migration pending).
