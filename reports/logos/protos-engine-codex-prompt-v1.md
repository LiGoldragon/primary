...
**3.1 Two lineages — do not conflate them.**

- **Lineage A (the literal "old" generator):** `repos/schema-language`
  (semantic decode) plus `repos/schema-rust` (`RustEmitter`,
  `src/lib.rs:62`, entry `emit_file_from_true_schema` at `:84`). This is the
  generator that today emits the real `// @generated by schema-rust` files into
  consumer `src/schema/` directories. It does **not** depend on core-schema. Its
  field names come from `schema-language` decode, whose pre-ban model still
  resolves same-typed-field collisions by an **author-supplied disambiguator
  stored as name data** — a spelling that is now illegal Protos.
- **Lineage B (the new self-hosting pipeline):** `core-schema` (v0.3.0,
  `2e47dec5`, field-name-ban codec) → `core-nomos` (macros + the deterministic
  name rule of §2.8) → `core-logos` → `textual-rust`. It intakes the new strict
  positional syntax, derives names by rule, and emits Rust proven byte-exact
  against Lineage A's real production goldens
  (`repos/core-nomos/tests/pipeline.rs`, `enriched.rs`), with
  `repos/language-engine-witness` running the delivered libraries as OS processes
  to prove the emitted Rust compiles and behaves.
...
3. **Self-host proof.** Drive the manifest through core-nomos → textual-rust,
   rebuild the engine from the emitted Rust, and accept on **working programs**
   (the `language-engine-witness` style: the rebuilt libraries run and pass their
   behavior tests). Do **not** accept on byte-equivalence against the current
   hand-written Rust, and do not reintroduce a byte-comparison oracle as the
   acceptance gate (psyche ruling; see §6).
...
Mark these in your work and escalate them; do not resolve them by implementation
choice:

1. **Which generator carries the bootstrap.** Recommended: consolidate on
   **Lineage B** (core-nomos / textual-rust) — it already intakes the new syntax,
   enforces the ban via core-schema `2e47dec5`, implements the deterministic name
   rule the old lineage lacks, and reproduces Lineage A byte-exact. Whether the
   literal old **Lineage A** (schema-rust / schema-language, which still carries
   the now-illegal author-disambiguator model) is then **migrated or retired** is
   the psyche's topology call. The vision's phrase "the old generator, adapted if
   necessary" is satisfied by Lineage B already reproducing schema-rust — but that
   reading should be confirmed, not assumed.
2. **The acceptance regime vs the existing byte-exact goldens.** The psyche ruled
   acceptance is **working programs, not byte-equivalence**, and "do not
   reintroduce a comparison oracle." The delivered Lineage-B validation currently
   proves itself with **byte-exact golden** tests against schema-rust output.
   Treat those goldens as inner-loop scaffolding; whether they are retired, or how
   they coexist with the working-program acceptance gate, is the psyche's to
   settle. Do not make byte-equivalence the bootstrap's acceptance gate.
3. **EncodedForm / encoded-identifier rename scope.** The terminology switch to
   "encoded form" / "encoded identifier" is accepted direction, but how far the
   rename reaches into existing `Core*` crate and symbol names is not settled.
4. **Protos crate topology.** Whether the delivered `structural-codec` (trait +
   ...
   (design-authority `[DECISION 2′]`). Do not rename crates on your own authority.
5. **The Nomos macro-definition surface, including the escape sigil.** Unsettled
   (§2.9). Do not exemplify or fix its spelling.
6. **Whether schema-language must adopt the ordinal rule** (or is retired with
   Lineage A). Its pre-ban author-disambiguator model is now-illegal syntax; the
   coexistence of two same-typed-field strategies is a joint, not your call.
