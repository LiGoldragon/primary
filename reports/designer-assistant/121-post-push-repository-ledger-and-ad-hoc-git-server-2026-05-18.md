...
```mermaid
flowchart TD
    local["agents / humans<br/>push to git.goldragon..."] --> gitserver["ad hoc Goldragon Git receive"]
...
- Better long-term if we want a minimal, auditable canonical Git
  receive layer.
- Pair with cgit or a tiny web viewer if browsing matters.
...
Stand up an ad hoc Goldragon Git service and make it the canonical
remote.
...
Current recommendation: **Forgejo if we want the ad hoc cloud to feel
like a useful forge quickly; Gitolite if we want the smallest canonical
receive authority. Do not start with Gerrit unless we decide to use its
review state as the actual coordination substrate.**

Name the deployment honestly:

- `ad-hoc-goldragon-cloud`
- `ad-hoc-goldragon-forge`
- `ad-hoc-goldragon-git`

The name should carry the warning that this is a bootstrap receive
point, not the final CriomOS cloud architecture.
...
- Or replace / reduce to Gitolite plus a smaller web view if
  Orchestrate and `repository-ledger` make the forge surface redundant.
...
The owner signal surface should be a separate contract and socket if
we keep applying the owner-signal security idea. Registering a watched
repository or changing signature policy is higher privilege than
