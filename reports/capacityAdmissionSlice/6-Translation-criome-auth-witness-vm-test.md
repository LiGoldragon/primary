...
- **prometheus declaration — partial blocker.** Two senses of "VmHost" diverge. The **prose** "authorized VM-testing host" rule is public and amendable (prometheus already appears in `scripts/*-on-prometheus`), so authorizing prometheus for this test is publicly doable via T5. But the **machine-consumed** cluster-data `VmHost` capability for prometheus lives in the public `goldragon/datom.nota` (the goldragon repo is public and not authorization-gated; only the SOPS-encrypted secret values it references are protected) (per `reports/operator/430-...`: append `(VmHost <guest-subnet> Available (Some <max>))` + a Pod), and `CriomOS-test-cluster/INTENT.md:97-99` + `checks/source-constraints.nix:15-24` forbid injecting prometheus/goldragon production facts into the synthetic fieldlab data. Recommendation: this witness needs only the prose authorization + the ssh-run path (T5), using fieldlab's existing VmHost `atlas` as the test's model anchor; the goldragon datom VmHost edit is NOT required here and defers with the capacity-admission work (the explicit non-goal). Confirm this split, or make the goldragon datom edit if the psyche wants prometheus carrying a real VmHost capability now.
