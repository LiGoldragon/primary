...
| `152-persona-engine-architecture-overview/` (meta-dir 0-9) | KEEP | Meta-report session unit per intent 231. Cited by `designer/292`, `designer/304`, `second-designer/153-158`, `second-operator/170` as the canonical Persona-engine-stack reference. Sub-reports 3 + 7 + 9 carry overview material no single ARCH covers. | Retires as a directory once every sub-report's substance has migrated to per-component ARCH files (still in-flight). |
