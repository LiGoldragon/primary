...
A second read-only diagnostic distinguished the boundary more precisely. Authoritative DNS resolution and the local Yggdrasil route for Zeus were correct, while TCP port 22 and verbose SSH both timed out during connection establishment. Prometheus was reachable over the same local Yggdrasil fabric, but it too could not reach Zeus over ICMP or TCP port 22. The authoritative Horizon has no Zeus LAN/link-local address, WireGuard identity, or ProxyJump endpoint, so there is no identity-safe alternate route to use for the pending production smoke.
