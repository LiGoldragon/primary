...
## Operator audit 234 — accepted corrections (folded into the implementation)

System-operator report 234 audited this report; its corrections are accepted, and the POC implementation uses the corrected design:

- **Safety-invariant correction (the greatest finding).** The ordinary `DeployClosure` as sketched still carries `ClusterName`+`NodeName`, so it can name a real node — the strong claim "ordinary cannot name a production node" overreaches. The correct, narrower invariant: ordinary can name a node *profile to build*, but cannot use a real node as the *live-mutation* or *generation-promotion* target. The implementation splits the nouns — a `NodeProfile` (configuration to build, ordinary) distinct from `ProductionNode` (live-mutation target, meta-only) — and the dangerous capability is `SystemAction Switch/Boot` against a real node, which is meta-only.
- **`AssertAgainst` renamed.** `Assert` is a Sema-class word; the ordinary verification root becomes `VerifyContained` (`AssertAgainst` may survive only as an internal/payload noun).
- **`EphemeralDroplet` is a confused-deputy boundary, not a cost footnote.** It stays `SubstrateUnavailable` until a typed lease/quota credential, daemon spend+count caps, a persisted SEMA lease table, restart reconciliation, and a quota-splitting negative test all exist. (The psyche accepts the bounded-spend-on-ordinary model this turn — *given* those guardrails.)
- **`LeaseSeconds` honesty.** The nonzero property is a schema/domain newtype; the configured maximum is daemon *policy*, not a schema-level "smart constructor."
- **`cpip` Clarify is optional, not required** — `mq5s` already states cpip's interface is lojix ordinary. The real drift is repo docs: `signal-lojix/INTENT.md` and `ARCHITECTURE.md` still list `Deploy`/`Pin`/`Unpin`/`Retire` as ordinary; those get synchronized when the implementation wave begins.
- **Report 157 is superseded** by this report (banner added to 157).
