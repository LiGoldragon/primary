...
| 2 | **Full-schema render** — `from_signal_schema_source()` parses the *complete* deployed `signal.schema` and `(Help)` renders **every** Input+Output root, and `(Help X)` works for every declared type — including the recursive `Domain` tree and `IntentEventStream`. | Largely covered (existing tests build the model from the full source; `render` is total so it cannot panic). **Recommend an explicit "all roots render, no error" assertion** as the deploy gate, so a future schema construct that the projector mishandles fails loudly here. |
