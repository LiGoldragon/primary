...
| 2 | **Full-schema render, from real data via schema-decode** — the gate must drive the model **generated from the real schema through the decode path** (`from_signal_schema_source()` → `from_schema_text(SIGNAL_SCHEMA_SOURCE)`), **never a hand-built fixture**. Over that decoded model, `(Help)` renders **every** Input+Output root and `(Help X)` works for every declared type — including the recursive `Domain` tree and `IntentEventStream`. | Largely covered (existing golden tests already call `from_signal_schema_source()`, so they decode real data; `render` is total). **Add an explicit "all roots render, no error" assertion** so a future schema construct the decoder/projector mishandles fails loudly here. This is the real point: a fixture tests only the renderer and can drift; decoding the live schema tests **decode → project → render** end-to-end and cannot drift (Help *is* the decoded schema). |
