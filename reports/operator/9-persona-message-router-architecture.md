...
| Component | Role |
|---|---|
| `signal-persona` | planned contract crate: frame envelope, handshake, version, typed wire records; not yet a repo |
...
`signal-persona` is the missing contract crate. It owns the wire types every
Persona daemon component speaks. Components exchange rkyv archives of
`signal-persona` types; NOTA is only a boundary projection.
...
`signal-persona` values, not NOTA strings.
...
```mermaid
flowchart TD
    contract["Step 0: scaffold signal-persona"]
    repos["component repos scaffolded"]
    msg["persona-message uses signal-persona"]
...
0. Scaffold `signal-persona`: frame envelope, handshake/version, message verbs,
...
2. Define the store command/result envelope as `signal-persona::Frame`. Use
...
```mermaid
flowchart LR
    contract["signal-persona round-trip tests"]
    nota["NOTA examples"]
...
|---|---|
| `signal-persona` frame round-trip | every `Request`, `Reply`, event, and record variant encodes and decodes |
| `signal-persona` handshake | protocol-compatible handshakes accept; incompatible versions reject |
