...
Current schema `main` now treats namespace declaration delimiters as:
...
Header roots remain the uniform route-selector form:
...
Verified parser paths on current `main`:
...
The Spirit schema fixtures and parser tests are already migrated to the
corrected syntax on `main`.
