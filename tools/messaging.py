...
class Reader:
 ...
 def value(self):
  ...
  if self.s[self.i:self.i+2]=='.{': self.i+=1; return Variant(head,self.value())
  return Bare(head)
...
def relay(text):
 ...
 body=root.body
 if isinstance(body,Group) and body.kind=='{' and len(body.values)==1: body=body.values[0]
 if not isinstance(body,Variant) or body.head!='Relay' or not isinstance(body.body,Group) or body.body.kind!='{' or len(body.body.values)!=3: raise ParseError('expected Relay.{ { from seat heard mode [recipients] } quote context }')
 env,quote,context=body.body.values
 if not isinstance(env,Group) or env.kind!='{' or len(env.values)!=5: raise ParseError('invalid relay envelope')
 frm,seat,heard,mode,recips=map(lambda p:p,env.values)
 ...
 if root.head=='LIVING' and mode=='unknown': raise ParseError('living relay needs typed or stt mode')
 ...
 return {'producer':root.head,'from':frm,'seat':seat,'heard':heard,'mode':mode,'recipients':recipients,'quote':quote.value,'context':context.value}
