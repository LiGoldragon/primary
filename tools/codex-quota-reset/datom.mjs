...
export function integer(value, what) {
  if (value?.bare === undefined || !/^(0|-?[1-9][0-9]*)$/.test(value.bare)) throw new Error(`${what} is not a datom integer`);
  return Number(value.bare);
}

export function symbol(value, what) {
  if (value?.bare === undefined) throw new Error(`${what} is not a datom symbol`);
  return value.bare;
}
