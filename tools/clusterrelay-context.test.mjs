...
let result = run('--source', source, '--source-id', 'target', '--source-flow-id', 'source-flow', '--executor-flow-id', 'cf7879', '--dry-run');
...
result = run('--source', source, '--queue-session-id', 'source-session', '--queue-timestamp', '2026-09-15T22:28:09.894Z', '--queue-content-sha256', queueHash, '--queue-line', '5', '--source-flow-id', 'source-flow', '--executor-flow-id', 'cf7879', '--dry-run');
assert.equal(result.status, 0);
...
result = run('--source', source, '--source-id', 'missing', '--source-flow-id', 'source-flow', '--executor-flow-id', 'cf7879', '--dry-run');
assert.equal(result.status, 2);
assert.match(result.stdout, /source id not found/);
fs.appendFileSync(source, '\n' + row('target', 'user', 'Duplicate'));
result = run('--source', source, '--source-id', 'target', '--source-flow-id', 'source-flow', '--executor-flow-id', 'cf7879', '--dry-run');
assert.equal(result.status, 2);
