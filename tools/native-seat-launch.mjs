...
const roles = {
  ...
  'field-sol-of-395aed': { model: 'gpt-5.6-sol', effort: 'medium', role: 'Field Sol', predecessor: '395aed', ancestor: '395aed', skills: ['spirit','main-flow','field','refresh','psyche','psyche-interraction','psyche-acquisition','behavior','correction','vocabulary','testing','subflow','edit-coordination','orchestrate','flow-evidence','prompt-crafting','codex-harness','herdr','messaging'], sourceManifest: ['flows/8565e8/reports/refresh-handoff.md','flows/8565e8/reports/morning-2026-09-20.md','flows/8565e8/reports/lojix-schema-compatibility-addendum.md'] },
...
function receiptOnlyResponse(turn) {
  const value=turn?.output_text??turn?.output?.text??turn?.response?.text;
  if(typeof value!=='string') throw new Error('verification refused: target first response is not an observed receipt-only response');
  // Field's audited startup prompt deliberately claims identity and writes its
  // short startup record in its first turn. Its final FIELD_STARTING/READY
  // line is the receipt for that explicitly authorized exception.
  if(/\bFIELD_(?:STARTING|READY)\s+flow=[0-9a-f]{6}\s+title="Field Luna [0-9a-f]{6}"/.test(value)) return;
  if(/\b(tool|task|delegat|flow[- ]?id)\b/i.test(value)) throw new Error('verification refused: target first response is not an observed receipt-only response');
}
...
function verifyRolloutReceipt(file,receipt) {
  ...
  const responseIndex=rows.reduce((found,r,i)=>i>inputIndex&&r.type==='event_msg'&&r.payload?.thread_id===receipt.threadId&&r.payload?.turn_id===receipt.turnId&&r.payload?.item?.type==='AgentMessage'?i:found,-1);
...
async function loadedThreadIds(call) {
  try { const list=await call('thread/loaded/list',{}); const ids=list?.data??list; return Array.isArray(ids)?ids:null; }
  catch { return null; }
}
async function ensureThreadLoaded(call, receipt) {
  const loaded=await loadedThreadIds(call);
  if (loaded?.includes(receipt.threadId)) return {loaded:true,resumed:false};
  await call('thread/resume',{threadId:receipt.threadId});
  const read=await call('thread/read',{threadId:receipt.threadId,includeTurns:false});
  const thread=read?.thread??read;
  const expected=receipt.canonicalTitle??receipt.provisionalTitle;
  if (thread?.id!==receipt.threadId || (expected && thread?.name!==expected)) throw new Error('resume refused: resumed native thread identity or title differs');
  const after=await loadedThreadIds(call);
  if (after && !after.includes(receipt.threadId)) throw new Error('resume refused: native thread is still not loaded');
  return {loaded:true,resumed:true};
}
async function resumeReceiptThread() {
  const receipt=readReceipt();
  if (!receipt.threadId || !['pending','verified','ready'].includes(receipt.status)) throw new Error('resume refused: receipt has no started native thread');
  const socket=receiptSocket(receipt);
  const result=await withRpc(socket,call=>ensureThreadLoaded(call,receipt));
  console.log(JSON.stringify({threadId:receipt.threadId,nativeTitle:receipt.canonicalTitle??receipt.provisionalTitle,...result,readiness:'native-thread-loaded'}));
}
...
function verifyVisualFooterReceipt() {
  ...
  const session=option('--herdr-session'), paneId=option('--herdr-pane'), agentName=option('--herdr-agent'), terminalId=option('--herdr-terminal');
  if(!session||!paneId||!agentName||!terminalId) throw new Error('visual-footer verification requires exact Herdr session, pane, agent, and terminal');
  const agent=herdrJson(session,'agent',agentName), pane=herdrJson(session,'pane',paneId);
  if(agent.name!==agentName||agent.pane_id!==paneId||agent.terminal_id!==terminalId||agent.agent!=='codex'||agent.interactive_ready!==true||pane.pane_id!==paneId||pane.terminal_id!==terminalId||path.resolve(agent.cwd)!==cwd||path.resolve(pane.cwd)!==cwd) throw new Error('visual-footer verification refuses a changed or non-ready Herdr binding');
...
function hasVerifiedVisualFooter(receipt) {
  ...
  return Boolean(evidence?.kind==='herdr-visible-footer' && evidence.threadId===receipt.threadId && evidence.endpoint===receipt.endpoint && /^gpt-6-luna low ·/.test(evidence.footer??''));
}
