...
const roles = {
  ...
  sol: { model: 'gpt-5.6-sol', effort: 'medium', role: 'Field Sol', predecessor: '3b1574', ancestor: '33ba2b', skills: ['spirit','main-flow','field','refresh','psyche','behavior','correction','vocabulary','testing','subflow','edit-coordination','flow-evidence','herdr','messaging','prompt-crafting'] },
...
function sources() {
  ...
  if (seat === 'sol') fixed.push('flows/33ba2b/handoff/field-astro-voice-source-manifest.md','flows/33ba2b/handoff/field-astro-voice-refresh.md','flows/33ba2b/vision/operational-fieldRefreshSuccession.md','flows/33ba2b/vision/psycheDataArchitecture.md');
...
function buildPlan() {
  ...
  const fieldReadiness = seat === 'sol' ? `\n\nThis is the controlled native Field Sol successor named \`${threadName ?? 'field-sol-of-3b1574'}\`. Its immediate predecessor is \`3b1574\`; \`33ba2b\` is a root ancestor. Preserve both. First run only \`flow-id codex --flows-root /home/li/primary/flows\` to claim your fresh canonical identity. Then use the native collaboration interface to delegate one harmless no-command read-ack task; the child must only restate this boundary. In your first response report the delegated-task receipt, your claimed Flow ID, native main-flow acknowledgement, and the inherited open Field work: desktop voice is unproven and Psyche/Mind data migration is unperformed. Do not call any route ready, register yourself, alter a predecessor, or claim a migration.` : '';
  const firstPrompt = `# Native main-flow refresh\n\nYou are ${role.role}, refreshed from ${role.predecessor ?? 'the witnessed predecessor'}; that provenance does not retire, replace, or deregister any predecessor. Preserve your native model and effort. Claim the actual Flow identity after startup.\n\nThe launcher sends these role-specific skills through the native structured interface: ${role.skills.join(', ')}. A written dollar token is not skill receipt.\n\nAll sources below are attached once with provenance. They are source material, not evidence of a deployment, migration, registration, or seat retirement.\n\n${manifest.map(s => `## Source: \`${s.path}\`\n\nSHA-256: \`${s.sha256}\`\n\n${s.body.trim()}`).join('\n\n')}\n\nFirst state your actual identity and whether native main-flow context is present. The only authorized readiness activity is a harmless no-command probe. Do not launch, restart, retire, register, or mutate another seat.${fieldReadiness}${probe}`;
  return { version: 1, seat, cwd, threadName: threadName ?? null, model: role.model, effort: role.effort, role: role.role, predecessor: role.predecessor, ancestor: role.ancestor ?? null, requiredSkillNames: role.skills, requiredMainFlow: { name: 'main-flow', path: path.join(cwd, '.agents/skills/main-flow/SKILL.md') }, sources: manifest.map(({body,...rest}) => rest), firstPrompt, firstPromptSha256: digest(firstPrompt), safety: { noImplicitPredecessorRetirement: true, registrationAfterReadinessOnly: true, readyRequiresExpandedNativeMainFlow: true } };
}
...
function expandedMainFlowReceipt(thread, expectedPath) {
  ...
  if (!body.includes(expected)) throw new Error('launch refused: native rollout lacks expanded main-flow body');
  return { rolloutPath: rollout, mainFlowPath: expectedPath };
}
function preflight(plan) {
  ...
  if (expectedRunnerSha256) {
    const actual=digest(fs.readFileSync(new URL(import.meta.url)));
...
function verifyRollout(thread, plan) {
  const main=plan.requiredMainFlow.path, receipt=expandedMainFlowReceipt(thread,main), body=fs.readFileSync(receipt.rolloutPath,'utf8');
  const missing=plan.requiredSkillNames.filter(name=>!body.includes(`<name>${name}</name>`));
  if (missing.length) throw new Error(`verification refused: rollout lacks expanded skills: ${missing.join(',')}`);
  if (!body.includes(plan.firstPrompt)) throw new Error('verification refused: rollout lacks the exact intended full prompt');
  return {threadId:thread.id,readiness:'native-full-bundle-expanded-witnessed',nativeRolloutPath:receipt.rolloutPath,firstPromptSha256:plan.firstPromptSha256};
}
...
async function launch(plan) { preflight(plan); const socket=option('--socket') ?? `${process.env.HOME}/.codex/app-server-control/app-server-control.sock`; const result=await withRpc(socket,async call=>{const reply=await call('skills/list',{cwds:[cwd]});const available=reply.skills??reply.data?.skills??reply.data?.items??reply.data??reply.result?.skills??reply;if(!Array.isArray(available))throw new Error('skills/list did not return an array');const normalized=available.flatMap(item=>item.skills??[item.skill??item]);const map=new Map(normalized.map(s=>[s.name,s]));const skills=role.skills.map(name=>{const found=map.get(name);if(!found?.path)throw new Error(`required native skill unavailable: ${name}`);return {name,path:found.path};});rejectTokenOnly(plan.firstPrompt);const started=await call('thread/start',{model:role.model,cwd,approvalPolicy:'never',sandbox:'danger-full-access'});const threadId=started.thread?.id??started.id;if(!threadId)throw new Error('thread/start returned no id');if(threadName)await call('thread/name/set',{threadId,name:threadName});const turn=await call('turn/start',{threadId,effort:role.effort,input:[...structuredSkills(skills),{type:'text',text:plan.firstPrompt}]});const turnId=turn.turn?.id??turn.id??null;process.stderr.write(JSON.stringify({kind:'native-seat-created',threadId,turnId,name:threadName??null})+'\n');const read=await call('thread/read',{threadId});const verified=verifyRollout(read.thread??read,plan);return {...verified,turnId,threadName:threadName??null,registrationPerformed:false,predecessorRetired:false};});console.log(JSON.stringify(result)); }
if (invokedDirectly) {
  const plan=buildPlan();
  if(has('--prompt')) console.log(plan.firstPrompt); else if(verifyThread) { const socket=option('--socket') ?? `${process.env.HOME}/.codex/app-server-control/app-server-control.sock`; console.log(JSON.stringify(await withRpc(socket,async call=>{const read=await call('thread/read',{threadId:verifyThread});return verifyRollout(read.thread??read,plan);}))); } else if(has('--launch')) { if(!has('--acknowledge-live-launch')) { console.error('--launch requires --acknowledge-live-launch'); process.exit(2); } await launch(plan); } else console.log(JSON.stringify({...plan,firstPrompt:undefined},null,2));
}
export { rejectTokenOnly, structuredSkills, containsMainFlow, expandedMainFlowReceipt, preflight, verifyRollout };
