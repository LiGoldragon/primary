...
assert.equal(service.publish({ flowId: 'core', harness: 'codex', observation: { state: idle.state, idleMinutes: idle.idleMinutes, sourceEvent: { kind: idle.source, identifier: idle.turnId }, unknownReason: null } }), true);
assert.equal(events.at(-1).kind, 'idleness-changed');
assert.equal(events.at(-1).observation.idleMinutes, 5);
assert.equal(service.publish({ flowId: 'core', harness: 'codex', observation: { state: idle.state, idleMinutes: idle.idleMinutes, sourceEvent: { kind: idle.source, identifier: idle.turnId }, unknownReason: null } }), false);
assert.throws(() => service.publish({ flowId: 'core', harness: 'claude', observation: { state: 'unknown', idleMinutes: null, sourceEvent: null, unknownReason: 'wrong-harness' } }), /harness mismatch/);
assert.throws(() => service.publish({ flowId: 'absent', harness: 'codex', observation: { state: 'unknown', idleMinutes: null, sourceEvent: null, unknownReason: 'unknown' } }), /unknown Flow/);
unsubscribe();
assert.equal(service.publish({ flowId: 'primary', harness: 'claude', observation: { state: 'approval-wait', idleMinutes: null, sourceEvent: null, unknownReason: 'approval-pending' } }), true);
