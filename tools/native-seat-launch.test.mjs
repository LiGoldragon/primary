...
for (const [seat,role,model,promptFile,promptBody] of [
  ...
  assert.equal(freshField.provisionalTitle,role);assert.equal(freshField.firstPromptSha256,crypto.createHash('sha256').update(`${fs.readFileSync(path.join(projectRoot,'tools','main-flow-mode','system-prompt.md'),'utf8')}\n${promptBody}`).digest('hex'));
  assert.equal(execFileSync(process.execPath,[tool,'--seat',seat,'--profile-file',profilePath,'--fresh','--cwd',dir,'--prompt'],{encoding:'utf8'}),`${fs.readFileSync(path.join(projectRoot,'tools','main-flow-mode','system-prompt.md'),'utf8')}\n${promptBody}\n`);
...
const mindAstraPlan=JSON.parse(execFileSync(process.execPath,[tool,'--seat','mind-astra-of-4b0f60','--profile-file',mindAstraProfile,'--predecessor','4b0f60','--cwd',dir],{encoding:'utf8'}));
assert.equal(mindAstraPlan.role,'Mind Astra'); assert.equal(mindAstraPlan.model,'gpt-6-astra'); assert.equal(mindAstraPlan.predecessor,'4b0f60');
const rejectedMindAstraPredecessor=spawnSync(process.execPath,[tool,'--seat','mind-astra-of-4b0f60','--profile-file',mindAstraProfile,'--predecessor','0ab019','--cwd',dir],{encoding:'utf8'});
assert.notEqual(rejectedMindAstraPredecessor.status,0);
...
let launchCalls=[],receiptPrecededTurn=false,launchedTurn,launchStarts=[];
const launchSocket=path.join(dir,'launch.sock'); let launchedTitle;
const launchServer=net.createServer(socket=>{let raw=Buffer.alloc(0),upgraded=false;const reply=(id,result)=>socket.write(serverFrame(JSON.stringify({jsonrpc:'2.0',id,result})));socket.on('data',data=>{raw=Buffer.concat([raw,data]);if(!upgraded){const end=raw.indexOf('\r\n\r\n');if(end<0)return;raw=raw.subarray(end+4);upgraded=true;socket.write('HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n\r\n');}while(raw.length>=2){let n=raw[1]&127,o=2;if(n===126){if(raw.length<4)return;n=raw.readUInt16BE(2);o=4;}if(raw.length<o+4+n)return;const mask=raw.subarray(o,o+4),body=Buffer.alloc(n);for(let i=0;i<n;i++)body[i]=raw[o+4+i]^mask[i%4];raw=raw.subarray(o+4+n);const request=JSON.parse(body);if(!request.id)continue;launchCalls.push(request.method);if(request.method==='skills/list')reply(request.id,['spirit','refresh','psyche','testing-flow-titles'].map(name=>({name,path:path.join(dir,'.agents/skills',name,'SKILL.md')})));else if(request.method==='thread/start'){launchStarts.push(request.params);reply(request.id,{thread:{id:launchedId}});}else if(request.method==='thread/name/set'){launchedTitle=request.params.name;reply(request.id,{});}else if(request.method==='turn/start'){receiptPrecededTurn=fs.existsSync(launchedReceipt)&&JSON.parse(fs.readFileSync(launchedReceipt,'utf8')).status==='created';launchedTurn=request;reply(request.id,{turn:{id:'first-turn'}});}else if(request.method==='thread/read'&&!request.params.includeTurns)reply(request.id,{thread:{id:launchedId,name:launchedTitle}});else if(request.method==='thread/read')socket.write(serverFrame(JSON.stringify({jsonrpc:'2.0',id:request.id,error:{code:-32601,message:'list_turns is not supported yet'}})));else reply(request.id,{});}});});
...
const firstLaunchCalls=[...launchCalls], firstReceiptPrecededTurn=receiptPrecededTurn, firstLaunchedTurn=launchedTurn;
const launchMindSol=extra=>new Promise(resolve=>{const child=spawn(process.execPath,[tool,'--seat','mind-sol','--profile-file',mindSolProfile,'--fresh','--cwd',dir,'--socket',launchSocket,'--expected-runner-sha256',runnerHash,'--launch','--acknowledge-live-launch',...extra]);let out='',err='';child.stdout.on('data',d=>out+=d);child.stderr.on('data',d=>err+=d);child.on('exit',code=>resolve({code,out,err}));});
const stockReceipt=path.join(dir,'stock-receipt.json');
const isolatedReceipt=path.join(dir,'isolated-receipt.json');
const isolatedLaunch=await launchMindSol(['--receipt',isolatedReceipt,'--main-seat']);
const startsBeforeMissing=launchStarts.length, callsBeforeMissing=launchCalls.length;
const missingPromptReceipt=path.join(dir,'missing-prompt-receipt.json');
const missingPrompt=await launchMindSol(['--receipt',missingPromptReceipt,'--main-flow-prompt',path.join(dir,'no-such-prompt.md')]);
launchServer.close();
assert.equal(launched.code,0,launched.err);
assert.deepEqual(firstLaunchCalls,['initialize','skills/list','thread/start','thread/name/set','thread/read','turn/start','thread/read']);
const mainFlowPromptPath=path.join(projectRoot,'tools','main-flow-mode','system-prompt.md');
assert.equal(launchStarts[0].config,undefined,'Codex launches keep stock base instructions unless isolated mode is explicitly requested');
assert.equal(JSON.parse(fs.readFileSync(launchedReceipt,'utf8')).mainFlowMode,null);
assert.ok(firstLaunchedTurn.params.input.at(-1).text.startsWith(`${fs.readFileSync(mainFlowPromptPath,'utf8')}\n`),'the one startup prompt begins with the main-flow text');
assert.equal(isolatedLaunch.code,0,isolatedLaunch.err);
assert.equal(launchStarts[1].config?.model_instructions_file,mainFlowPromptPath,'explicit isolated mode replaces the Codex base instructions');
assert.equal(JSON.parse(fs.readFileSync(isolatedReceipt,'utf8')).mainFlowMode.sha256,crypto.createHash('sha256').update(fs.readFileSync(mainFlowPromptPath,'utf8')).digest('hex'));
assert.notEqual(missingPrompt.code,0);
assert.match(missingPrompt.err,/main-flow system prompt file missing/);
assert.equal(launchCalls.length,callsBeforeMissing,'a missing main-flow prompt is refused before any app-server call');
assert.equal(launchStarts.length,startsBeforeMissing);
assert.equal(fs.existsSync(missingPromptReceipt),false);
assert.equal(firstReceiptPrecededTurn,true,'native receipt must exist before the first target turn');
assert.equal(JSON.parse(fs.readFileSync(launchedReceipt,'utf8')).threadId,launchedId);
assert.ok(firstLaunchedTurn.params.input.some(input=>input.type==='skill'&&input.name==='main-flow'));
...
const fieldStarting=structuredClone(good);fieldStarting.thread.turns[0].output_text='I will delegate registration.\nFIELD_STARTING flow=e71dab title="Field Luna e71dab"';
assert.equal(verifyReceipt(fieldStarting,receiptShape).readiness,'native-context-verified-title-pending');
...
assert.deepEqual(clientForModel('gpt-6-astra','/home/li'),{command:'codex-next',expectedPath:'/home/li/.nix-profile/bin/codex-next',endpoint:'/home/li/.codex-next/app-server-control/app-server-control.sock'});
