...
const stockLaunch=await launchMindSol(['--receipt',stockReceipt,'--no-main-seat']);
...
assert.equal(launchStarts[0].config?.model_instructions_file,mainFlowPromptPath,'a main seat replaces the Codex base instructions with the main-flow prompt');
assert.equal(JSON.parse(fs.readFileSync(launchedReceipt,'utf8')).mainFlowMode.sha256,crypto.createHash('sha256').update(fs.readFileSync(mainFlowPromptPath)).digest('hex'));
assert.equal(stockLaunch.code,0,stockLaunch.err);
assert.equal(launchStarts[1].config,undefined,'a non-main seat keeps the stock Codex base instructions');
assert.equal(JSON.parse(fs.readFileSync(stockReceipt,'utf8')).mainFlowMode,null);
