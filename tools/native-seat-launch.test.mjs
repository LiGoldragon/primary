...
fs.writeFileSync(rolloutPath, plan.requiredSkillNames.map(name=>`<skill>\n<name>${name}</name>\n<path>${path.join(dir,'.agents/skills',name,'SKILL.md')}</path>\nbody\n</skill>`).join('\n')+'\n'+prompt);
...
assert.equal(launched.code,0,launched.err); assert.ok(turnStart.params.input.some(i=>i.type==='skill'&&i.name==='main-flow')); assert.equal(JSON.parse(launched.out).readiness,'native-full-bundle-expanded-witnessed');
