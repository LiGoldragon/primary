...
import {verifyReceipt,verifyRolloutReceipt,activationPrompt} from './native-seat-launch.mjs';
...
fs.writeFileSync(freshProfile,JSON.stringify({name:'fresh-luna',model:'gpt-5.6-luna',effort:'low',role:'Fresh Luna',fresh:true,predecessor:null,ancestor:null,skills:['spirit','main-flow','refresh','psyche'],sourceManifest:['Vision/flowNexus.md'],...audited()}));
...
fs.writeFileSync(freshProfile,JSON.stringify({name:'fresh-luna',model:'gpt-5.6-luna',effort:'low',role:'Fresh Luna',fresh:true,predecessor:null,ancestor:null,skills:['spirit','main-flow','refresh','psyche'],sourceManifest:['Vision/flowNexus.md'],nativeTitle:'Fixture native title'}));
...
fs.writeFileSync(mindSolProfile,JSON.stringify({name:'mind-sol',model:'gpt-5.6-sol',effort:'medium',role:'Mind Medium',fresh:true,predecessor:null,ancestor:null,skills:['spirit','main-flow','refresh','psyche'],sourceManifest:['Vision/flowNexus.md'],...audited()}));
...
const fieldSolProfileValue={name:'field-sol-of-7091ea',model:'gpt-5.6-sol',effort:'medium',role:'Field Sol',fresh:false,predecessor:'7091ea',ancestor:'7091ea',skills:['spirit','main-flow','field','refresh','psyche'],sourceManifest:['Vision/flowNexus.md'],...audited()};
...
const fieldAstraProfileValue={name:'field-astra-of-6db4fe',model:'gpt-6-astra',effort:'medium',role:'Field Astra',fresh:false,predecessor:'6db4fe',ancestor:'6db4fe',skills:['spirit','main-flow','field','refresh','psyche'],sourceManifest:['Vision/flowNexus.md'],...audited()};
...
assert.match(refused.err,/only an authorized Mind Sol, Field Sol, or Field Astra profile/);
...
const launchServer=net.createServer(socket=>{let raw=Buffer.alloc(0),upgraded=false;const reply=(id,result)=>socket.write(serverFrame(JSON.stringify({jsonrpc:'2.0',id,result})));socket.on('data',data=>{raw=Buffer.concat([raw,data]);if(!upgraded){const end=raw.indexOf('\r\n\r\n');if(end<0)return;raw=raw.subarray(end+4);upgraded=true;socket.write('HTTP/1.1 101 Switching Protocols\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n\r\n');}while(raw.length>=2){let n=raw[1]&127,o=2;if(n===126){if(raw.length<4)return;n=raw.readUInt16BE(2);o=4;}if(raw.length<o+4+n)return;const mask=raw.subarray(o,o+4),body=Buffer.alloc(n);for(let i=0;i<n;i++)body[i]=raw[o+4+i]^mask[i%4];raw=raw.subarray(o+4+n);const request=JSON.parse(body);if(!request.id)continue;launchCalls.push(request.method);if(request.method==='skills/list')reply(request.id,['spirit','main-flow','refresh','psyche'].map(name=>({name,path:path.join(dir,'.agents/skills',name,'SKILL.md')})));else if(request.method==='thread/start')reply(request.id,{thread:{id:launchedId}});else if(request.method==='thread/name/set'){launchedTitle=request.params.name;reply(request.id,{});}else if(request.method==='turn/start'){receiptPrecededTurn=fs.existsSync(launchedReceipt)&&JSON.parse(fs.readFileSync(launchedReceipt,'utf8')).status==='created';launchedTurn=request;reply(request.id,{turn:{id:'first-turn'}});}else if(request.method==='thread/read'&&!request.params.includeTurns)reply(request.id,{thread:{id:launchedId,name:launchedTitle}});else if(request.method==='thread/read')socket.write(serverFrame(JSON.stringify({jsonrpc:'2.0',id:request.id,error:{code:-32601,message:'list_turns is not supported yet'}})));else reply(request.id,{});}});});
...
assert.equal(JSON.parse(adopted.out).readiness,'native-full-bundle-rollout-witnessed');
