...
export const renderSituationReport = (input) => {
  ...
  if (!primary || !spark || !credits || typeof input.observedAt !== "string") {
    throw new Error("invalid read-only quota fixture");
  }

  const observedAt = new Date(input.observedAt);
  const resetsAt = new Date(primary.resetsAt);
  const windowMs = primary.windowMinutes * 60_000;
  ...
  const elapsedPercent = Math.round((elapsedDays / 7) * 100);
