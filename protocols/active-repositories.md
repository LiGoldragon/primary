...
| `cloud` | `/git/github.com/LiGoldragon/cloud` | Live runtime repo for provider API management: ordinary + owner Unix-socket daemon over a synchronous sema-engine `Store`, with DigitalOcean / Hetzner / Cloudflare adapters; DigitalOcean Tier-1 lifecycle is live-proven (reports `cloud-designer/64`, `66`). Birth bead `primary-kbmi` is closed; live land + hardening is tracked by `primary-hpkj`. In the kameo-fork camp (702 fleet split-brain). |
