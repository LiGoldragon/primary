...
| `cloud` | `/git/github.com/LiGoldragon/cloud` | Runtime repo for provider API management: ordinary + owner Unix-socket daemon (single `EngineActor` over a synchronous sema-engine `Store`) with DigitalOcean / Hetzner / Cloudflare adapters. DigitalOcean is the only compute provider shipped in a daemon build (`apps.daemon-digitalocean`); its Tier-1 lifecycle was exercised live once via an `#[ignore]` adapter test (report `cloud-designer/64`) — no reproducible artifact witnesses a live *provision* yet (audit `68` risk 1). Birth bead `primary-kbmi` closed; land + hardening tracked by `primary-hpkj`; structural follow-ups in audit `68`. In the kameo-fork camp (702 split-brain). |
