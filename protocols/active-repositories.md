...
| `persona-introspect` (**planned**) | `github:LiGoldragon/persona-introspect` (not yet created) | High-privilege read/projection component. Talks to each supervised component daemon over Signal to fan in typed inspectable records, projects to NOTA for humans/agents/UIs. Does not directly open any other component's redb. Per `~/primary/reports/designer/146-introspection-component-and-contract-layer.md`. |
| `signal-persona-introspect` (**planned**) | `github:LiGoldragon/signal-persona-introspect` (not yet created) | Central introspection envelope contract: `IntrospectionRequest` / `IntrospectionReply` / `IntrospectionSubscription` / `IntrospectionEvent` / `IntrospectionProjection`. Per /146 §2. Component-specific introspection records (when a component needs them) live either in the existing `signal-persona-<X>` crate or in a sibling `signal-persona-<X>-introspect` crate. |
