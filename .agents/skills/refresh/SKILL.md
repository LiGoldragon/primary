...
Assemble each successor's first prompt programmatically, never by freehand reconstruction: spirit, applicable intent and vision, raw vision with its context and transcript provenance, open items, witnessed knowledge, clearly labelled inferences, relevant distillation, and the required skills loaded through the skill interface. Each successor claims a distinct native `FLOW_ID`, records its native-start receipt, and remembers its immediate predecessor at depth one.
