...
`FLOW_ID=<self> hm-send TARGET BODY [--wait-presented] [--hold-seconds N] [--pane PANE]` sends the complete machine body as `#msg [sender machine-prose]`. `FLOW_ID=<self> hm-send TARGET --psyche CONTEXT VERBATIM [--wait-presented] [--hold-seconds N] [--pane PANE]` sends the complete living words as `#psyche [sender context verbatim]`: put the context before the exact verbatim and do not truncate or summarize it. Pass a body or the context and verbatim, never a prebuilt envelope. A body that parses as one complete `#msg` or `#psyche` form is rejected as nesting; ordinary prose may mention either tag. Large submissions can arrive at Claude as `pasted_content` and are acceptable.
