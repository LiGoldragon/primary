...
class Role:
    kind: str      # 'Main' | 'Sub'
    value: str     # a Layer or a SubflowKind

    @property
    def name(self) -> str:
        return (_snake(self.value) + '-main') if self.kind == 'Main' \
            else ('subflow-' + _snake(self.value))
...
def _role(form) -> Role:
    v = dr.variant(form, ('Main', 'Sub'), 'Role')
    allowed = LAYER if v.text == 'Main' else SUBFLOW_KIND
    inner = dr.variant(v.payload, allowed, 'Layer' if v.text == 'Main' else 'SubflowKind')
