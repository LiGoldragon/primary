...
The psyche logging system is in place and committed. The awareness
protocol is working — four aspects active (steward, designer,
discipline, writer). Context handover is phased out.

The skills repo has a generator. Skills are flat source files in
`skills/`, controlled by manifests, assembled by a Rust CLI into
harness-native outputs. Never write directly to generated output
directories. The generator must run clean before any deployment.

Awareness, psyche-grasp, and the rewritten psyche-interraction are
now in the skills repo and deployed. The absorbed skills (design-log,
psyche-vision, spirit-log) are removed from source and manifests.

The awareness skill's description is wrong — `~` (null) does not
make it non-agent-visible. The mechanism for controlling skill
visibility in harnesses needs research. This is unresolved.
...
- Skills are flat sources in `skills/` in LiGoldragon/skills; never
  write directly to generated harness directories.
- **Never prescribe implementation without verified knowledge of
  the mechanism.** Agents must research how something actually works
  before acting. Assuming a mechanism from naming or convention alone
  is how the awareness description got wrong. This lesson needs to
  become doctrine for all agents.

## Threads I'm tracking

- Awareness skill description: `~` is wrong; need to research how
  harnesses control skill visibility, then fix properly
- Spirit skill replacing tenets — not yet touched
- Claude Code teammateDefaultModel — unresolved, mechanism unknown
- 25 items blocked on psyche (see state-of-everything report)
- 3 spirit-vertical gating rulings are the most consequential blockers
- Designer's 11 overnight decisions need psyche review
...
- How harnesses discover/surface skills and what controls visibility
- Why this session ran with 200k context despite being labeled 1M
...
- f0bdaf3f — built psyche logging system, rewrote psyche-interraction,
  created psyche-acquisition role, absorbed three logging skills,
  Codex worker to xhigh (2026-08-07)
- (this session) — three-wave deep state-of-everything audit (11
  subagents, visual report published), secured 38 uncommitted files on
  detached HEAD (6973354c), ported skills to skills repo with generator
  (9aeb2007). Lesson learned: acted on skill visibility without
  researching the mechanism. Psyche corrected — agents must verify
  how things work before prescribing solutions. (2026-08-08)
