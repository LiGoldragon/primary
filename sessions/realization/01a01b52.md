...
- Chroma is accepting GeoClue fixes with approximately 26 km accuracy. Current Chroma validates freshness and finiteness but does not reject poor accuracy. An unmerged Chroma branch rejects IP fallback and accuracy worse than 1 km; it is evidence, not the selected fix.
...
- Which component selected or retained 2700 K, and whether Chroma and Noctalia are competing over the relay.
...
- Whether “status bar” means Noctalia tray/status-notifier items, Noctalia palette changes, or application header-bar theming.
- Which writer leaves GTK and icon themes dark after Chroma reports light.
